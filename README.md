...
- 直接双击根目录 `package-windows.bat`
- 默认自动递增 `patch` 版本号，可在弹窗里切换 `minor`、`major` 或手动输入版本
- Windows 安装包输出目录：`src-tauri/target/release/bundle/nsis/`
- 命令行备用入口：`npm run package:windows`
