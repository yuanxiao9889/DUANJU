...
import {
  ...
  type ImageCompareRestorableNodeSnapshot,
...
const nodeIndexByIdCache = new WeakMap<CanvasNode[], Map<string, number>>();

function getNodeIndexByIdMap(nodes: CanvasNode[]): Map<string, number> {
  const cached = nodeIndexByIdCache.get(nodes);
  if (cached) {
    return cached;
  }
  ...
  nodeIndexByIdCache.set(nodes, map);
  return map;
}
...
function applyNodeChangesLocal(
  ...
  if (changes.every((change) => change.type === 'select' || change.type === 'position')) {
    ...
    const nodeIndexById = getNodeIndexByIdMap(nodes);
  ...
  if (changes.every((change): change is CanvasNodePositionChange => change.type === 'position')) {
    ...
    for (const change of changeById.values()) {
      const index = nodeIndexById.get(change.id);
      if (typeof index !== 'number') {
        continue;
      }
...
const DERIVED_NODE_MAX_COLUMN_SEARCH = 16;
...
type ImageCompareMergeFailureReason =
  | 'missingNode'
  | 'sameNode'
  | 'invalidNode'
  | 'missingImage';

interface ImageCompareMergeResult {
  status: 'success' | ImageCompareMergeFailureReason;
...
interface CanvasState {
  ...
  mergeImageNodesForCompare: (sourceNodeId: string, targetNodeId: string) => ImageCompareMergeResult;
  splitImageCompareNode: (compareNodeId: string) => boolean;

  deleteNode: (nodeId: string) => void;
  deleteNodes: (nodeIds: string[]) => void;
  groupNodes: (nodeIds: string[]) => string | null;
  layoutGroupNode: (groupNodeId: string) => boolean;
  fitGroupNodeToChildren: (groupNodeId: string) => boolean;
...
function normalizeNodes(rawNodes: CanvasNode[]): CanvasNode[] {
  const normalizedNodes = rawNodes
    .map((node) => {
      ...
      const normalizedNodeType = node.type === CANVAS_NODE_TYPES.directorWorkPackage
        ? CANVAS_NODE_TYPES.scriptAssetExtract
        : isKnownCanvasNodeType(node.type)
      ...
      if (normalizedNodeType === CANVAS_NODE_TYPES.scriptPlotLine) {
        Object.assign(
          mergedData,
          normalizeScriptPlotLineNodeData(mergedData as ScriptPlotLineNodeData)
        );
      }
      ...
      if (
        normalizedNodeType === CANVAS_NODE_TYPES.scriptAssetExtract
        || normalizedNodeType === CANVAS_NODE_TYPES.directorWorkPackage
      ) {
        Object.assign(
          mergedData,
      ...
      if (normalizedNodeType === CANVAS_NODE_TYPES.directorStoryboardReference) {
        Object.assign(
          mergedData,
          normalizeDirectorStoryboardReferenceNodeData(
            mergedData as DirectorStoryboardReferenceNodeData
          )
        );
      }
      ...
      if (normalizedNodeType === CANVAS_NODE_TYPES.adScriptReference) {
        Object.assign(
          mergedData,
          normalizeAdScriptReferenceNodeData(mergedData as AdScriptReferenceNodeData)
        );
      }

      if (normalizedNodeType === CANVAS_NODE_TYPES.commerceProduct) {
        Object.assign(
          mergedData,
          normalizeCommerceProductNodeData(mergedData as CommerceProductNodeData)
        );
      }

      if (normalizedNodeType === CANVAS_NODE_TYPES.commerceBrief) {
        Object.assign(
          mergedData,
      ...
      if (normalizedNodeType === CANVAS_NODE_TYPES.commerceResultGroup) {
        Object.assign(
          mergedData,
          normalizeCommerceResultGroupNodeData(mergedData as CommerceResultGroupNodeData)
        );
      }
      ...
      if (normalizedNodeType === CANVAS_NODE_TYPES.imageCompare) {
        Object.assign(
          mergedData,
...
function createNodePatchSnapshot(
  ...
  for (const nodeId of nodeIds) {
    ...
    const previousNodeIndex = nodeIndexById.get(nodeId);
    const previousNode = typeof previousNodeIndex === 'number'
...
function isFiniteNodeDimension(value: unknown): value is number {
  return typeof value === 'number' && Number.isFinite(value) && value > 0;
}

function buildImageCompareImageSnapshot(node: CanvasNode): ImageCompareNodeImageSnapshot | null {
  ...
  return {
    ...
    sourceNodeType:
      node.type === CANVAS_NODE_TYPES.upload || node.type === CANVAS_NODE_TYPES.exportImage
        ? node.type
        : null,
    imageUrl: imageSource.imageUrl,
    previewImageUrl: imageSource.previewImageUrl,
    aspectRatio: imageSource.aspectRatio,
    displayName:
      typeof node.data.displayName === 'string' && node.data.displayName.trim().length > 0
        ? node.data.displayName.trim()
        : null,
  };
}

function buildImageCompareRestorableNodeSnapshot(
  node: CanvasNode,
  originalIndex: number
): ImageCompareRestorableNodeSnapshot | null {
  if (node.type !== CANVAS_NODE_TYPES.upload && node.type !== CANVAS_NODE_TYPES.exportImage) {
    return null;
  }
  ...
  return {
    ...
    width: isFiniteNodeDimension(node.width) ? node.width : undefined,
    height: isFiniteNodeDimension(node.height) ? node.height : undefined,
    originalIndex,
    data: cloneCanvasValue(node.data as UploadImageNodeData | ExportImageNodeData),
  };
}

function restoreImageCompareNodeSnapshot(
  ...
  const restoredNode: CanvasNode = {
    ...
    data: cloneCanvasValue(snapshot.data),
    selected: false,
    parentId: undefined,
    extent: undefined,
  };

  if (isFiniteNodeDimension(snapshot.width) || isFiniteNodeDimension(snapshot.height)) {
    restoredNode.width = isFiniteNodeDimension(snapshot.width) ? snapshot.width : undefined;
    restoredNode.height = isFiniteNodeDimension(snapshot.height) ? snapshot.height : undefined;
    restoredNode.style = {
      ...(isFiniteNodeDimension(snapshot.width) ? { width: snapshot.width } : {}),
      ...(isFiniteNodeDimension(snapshot.height) ? { height: snapshot.height } : {}),
    };
  }

  return applyDefaultNodeSize(restoredNode, restoredNode.data);
}
...
function withMultiAngleImageDefaultSize(node: CanvasNode): CanvasNode {
  return {
    ...node,
    width: MULTI_ANGLE_IMAGE_NODE_DEFAULT_WIDTH,
    height: MULTI_ANGLE_IMAGE_NODE_DEFAULT_HEIGHT,
    style: {
      ...(node.style ?? {}),
      width: MULTI_ANGLE_IMAGE_NODE_DEFAULT_WIDTH,
      height: MULTI_ANGLE_IMAGE_NODE_DEFAULT_HEIGHT,
...
function withScriptCharacterDefaultSize(node: CanvasNode): CanvasNode {
  return {
    ...node,
...
function withScriptLocationDefaultSize(node: CanvasNode): CanvasNode {
  return {
    ...node,
...
function withScriptItemDefaultSize(node: CanvasNode): CanvasNode {
  return {
    ...node,
...
function withScriptPlotLineDefaultSize(node: CanvasNode): CanvasNode {
  return {
    ...node,
    width: SCRIPT_PLOT_LINE_NODE_DEFAULT_WIDTH,
    height: SCRIPT_PLOT_LINE_NODE_DEFAULT_HEIGHT,
    style: {
      ...(node.style ?? {}),
      width: SCRIPT_PLOT_LINE_NODE_DEFAULT_WIDTH,
      height: SCRIPT_PLOT_LINE_NODE_DEFAULT_HEIGHT,
...
function withScriptAssetExtractDefaultSize(node: CanvasNode): CanvasNode {
  return {
    ...node,
    width: SCRIPT_ASSET_EXTRACT_NODE_DEFAULT_WIDTH,
    height: SCRIPT_ASSET_EXTRACT_NODE_DEFAULT_HEIGHT,
    style: {
      ...(node.style ?? {}),
      width: SCRIPT_ASSET_EXTRACT_NODE_DEFAULT_WIDTH,
      height: SCRIPT_ASSET_EXTRACT_NODE_DEFAULT_HEIGHT,
...
function withDirectorStoryboardReferenceDefaultSize(node: CanvasNode): CanvasNode {
  return {
    ...node,
    width: DIRECTOR_STORYBOARD_REFERENCE_NODE_DEFAULT_WIDTH,
    height: DIRECTOR_STORYBOARD_REFERENCE_NODE_DEFAULT_HEIGHT,
    style: {
      ...(node.style ?? {}),
      width: DIRECTOR_STORYBOARD_REFERENCE_NODE_DEFAULT_WIDTH,
      height: DIRECTOR_STORYBOARD_REFERENCE_NODE_DEFAULT_HEIGHT,
...
function withAdScriptReferenceDefaultSize(node: CanvasNode): CanvasNode {
  return {
    ...node,
    width: AD_SCRIPT_REFERENCE_NODE_DEFAULT_WIDTH,
    height: AD_SCRIPT_REFERENCE_NODE_DEFAULT_HEIGHT,
    style: {
      ...(node.style ?? {}),
      width: AD_SCRIPT_REFERENCE_NODE_DEFAULT_WIDTH,
      height: AD_SCRIPT_REFERENCE_NODE_DEFAULT_HEIGHT,
...
function withImageCompareDefaultSize(node: CanvasNode): CanvasNode {
  return {
    ...node,
    width: IMAGE_COMPARE_NODE_DEFAULT_WIDTH,
    height: IMAGE_COMPARE_NODE_DEFAULT_HEIGHT,
    style: {
      ...(node.style ?? {}),
      width: IMAGE_COMPARE_NODE_DEFAULT_WIDTH,
      height: IMAGE_COMPARE_NODE_DEFAULT_HEIGHT,
...
function applyDefaultNodeSize(node: CanvasNode, data: CanvasNodeData): CanvasNode {
  ...
  if (isCommerceStageNodeType(node.type)) {
    const resolvedWidth =
  ...
  if (node.type === CANVAS_NODE_TYPES.multiAngleImage) {
    const resolvedWidth =
      resolveNumericNodeDimension(node.width)
      ?? resolveNumericNodeDimension(node.style?.width);
    const resolvedHeight =
      ...
      ?? resolveNumericNodeDimension(node.style?.height);
    if (resolvedWidth === null || resolvedHeight === null) {
      return withMultiAngleImageDefaultSize(node);
    }
  }
  ...
  if (node.type === CANVAS_NODE_TYPES.imageCompare) {
    const resolvedWidth =
      resolveNumericNodeDimension(node.width)
      ?? resolveNumericNodeDimension(node.style?.width);
    const resolvedHeight =
  ...
  if (node.type === CANVAS_NODE_TYPES.directorStoryboardReference) {
    ...
    const resolvedHeight =
      ...
      ?? resolveNumericNodeDimension(node.style?.height);
    if (resolvedWidth === null || resolvedHeight === null) {
      return withDirectorStoryboardReferenceDefaultSize(node);
    }
  }
  ...
  if (node.type === CANVAS_NODE_TYPES.adScriptReference) {
    const resolvedWidth =
      resolveNumericNodeDimension(node.width)
      ?? resolveNumericNodeDimension(node.style?.width);
    const resolvedHeight =
      ...
      ?? resolveNumericNodeDimension(node.style?.height);
    if (resolvedWidth === null || resolvedHeight === null) {
      return withAdScriptReferenceDefaultSize(node);
    }
  }
  ...
  if (node.type === CANVAS_NODE_TYPES.scriptCharacter) {
    ...
    const resolvedHeight =
      ...
      ?? resolveNumericNodeDimension(node.style?.height);
    if (resolvedWidth === null || resolvedHeight === null) {
      return withScriptCharacterDefaultSize(node);
    }
  }
  ...
  if (node.type === CANVAS_NODE_TYPES.scriptLocation) {
    ...
    const resolvedHeight =
      ...
      ?? resolveNumericNodeDimension(node.style?.height);
    if (resolvedWidth === null || resolvedHeight === null) {
      return withScriptLocationDefaultSize(node);
    }
  }
  ...
  if (node.type === CANVAS_NODE_TYPES.scriptItem) {
    ...
    const resolvedHeight =
      ...
      ?? resolveNumericNodeDimension(node.style?.height);
    if (resolvedWidth === null || resolvedHeight === null) {
      return withScriptItemDefaultSize(node);
    }
  }
  ...
  if (node.type === CANVAS_NODE_TYPES.scriptPlotLine) {
    ...
    const resolvedHeight =
      ...
      ?? resolveNumericNodeDimension(node.style?.height);
    if (resolvedWidth === null || resolvedHeight === null) {
      return withScriptPlotLineDefaultSize(node);
    }
  }
...
function collidesWithExistingNodes(
  ...
  const candidateRect = createCanvasRect(x, y, width, height);
  return nodes.some((node) => {
    if (ignoredNodeIds?.has(node.id)) {
      return false;
    }

    return rectIntersects(candidateRect, getCanvasNodeRect(node, nodeMap), 8);
  });
}
...
function resolvePreferredDerivedColumnPosition(
  ...
  for (let columnIndex = 0; columnIndex < DERIVED_NODE_MAX_COLUMN_SEARCH; columnIndex += 1) {
    for (let rowIndex = 0; rowIndex < DERIVED_NODE_MAX_ROWS_PER_COLUMN; rowIndex += 1) {
      ...
      if (candidateX < minAcceptedColumnX) {
        continue;
      }
      if (
...
function resolveBatchDerivedNodePosition(
  ...
  return {
    x: anchorX + DERIVED_NODE_MAX_COLUMN_SEARCH * stepX,
    y: anchorY,
  };
}
...
function resolveNodeCreationDefaults(
  ...
  const {
    ...
    lastMidjourneyAspectRatio,
    lastMidjourneyVersionPreset,
    lastMidjourneyRawMode,
  ...
  if (type === CANVAS_NODE_TYPES.mj) {
    const mjData = data as Partial<MjNodeData>;

    return {
      ...mjData,
      aspectRatio: mjData.aspectRatio ?? lastMidjourneyAspectRatio,
      rawMode: mjData.rawMode ?? lastMidjourneyRawMode,
      versionPreset: mjData.versionPreset ?? lastMidjourneyVersionPreset,
      personalizationCodes:
        mjData.personalizationCodes ?? lastMidjourneyPersonalizationCodes,
      advancedParams: mjData.advancedParams ?? lastMidjourneyAdvancedParams,
    } as Partial<CanvasNodeData>;
  }
...
export const useCanvasStore = create<CanvasState>((set, get) => ({
    ...
    set((state) => {
      ...
      if (hasInteractionMove && !nextDragHistorySnapshot) {
        nextDragHistorySnapshot = interactionNodeIds.length > 0
          ? createNodePatchSnapshot(state.nodes, interactionNodeIds, state.edges)
  ...
  setCanvasViewportSize: (size) => {
    set((state) => (
      state.canvasViewportSize.width === size.width
        && state.canvasViewportSize.height === size.height
        ? state
        : { canvasViewportSize: size }
    ));
  },

  openImageViewer: (imageUrl, imageList = [], metadata = null) => {
  ...
  addDerivedUploadNode: (sourceNodeId, imageUrl, aspectRatio, previewImageUrl, options) => {
    const state = get();
  ...
  addDerivedExportNode: (sourceNodeId, imageUrl, aspectRatio, previewImageUrl, options) => {
    ...
    const sourceGenerationSummary = sourceNode?.type === CANVAS_NODE_TYPES.exportImage
      ? normalizeExportImageGenerationSummary(
        (sourceNode.data as ExportImageNodeData).generationSummary
      )
      : null;
  ...
  addDerivedVideoNode: (
    ...
    const nodeMap = new Map(state.nodes.map((node) => [node.id, node] as const));
    const parentGroupId = resolveInheritedGroupParentId(sourceNode, nodeMap);
    const sourceVideo = resolveSingleVideoConnectionSource(sourceNode);
    const resolvedAspectRatio = aspectRatio || sourceVideo?.aspectRatio || '16:9';
    const sourceSize = sourceNode ? getNodeSize(sourceNode) : null;
    const fallbackSize = {
      width: JIMENG_VIDEO_RESULT_NODE_DEFAULT_WIDTH,
      height: JIMENG_VIDEO_RESULT_NODE_DEFAULT_HEIGHT,
    };
    const derivedSize = sourceSize
      ? {
        width: Math.max(1, Math.round(sourceSize.width)),
        height: Math.max(1, Math.round(sourceSize.height)),
      }
    ...
    const videoNodeData: Partial<CanvasNodeData> = {
      videoUrl,
      previewImageUrl: previewImageUrl ?? null,
    ...
    let node = canvasNodeFactory.createNode(CANVAS_NODE_TYPES.video, position, videoNodeData);
    node.width = derivedSize.width;
    node.height = derivedSize.height;
    node.style = {
      ...(node.style ?? {}),
  ...
  updateNodeData: (nodeId, data, options) => {
    set((state) => {
      ...
      const nextNodes = state.nodes.map((node) => {
        ...
        if (node.type === CANVAS_NODE_TYPES.commerceProduct) {
          Object.assign(
            mergedData,
            normalizeCommerceProductNodeData(mergedData as CommerceProductNodeData)
          );
        }
        if (node.type === CANVAS_NODE_TYPES.commerceBrief) {
          Object.assign(
            mergedData,
            normalizeCommerceBriefNodeData(mergedData as CommerceBriefNodeData)
          );
      ...
      if (affectedChapterIds.size > 0) {
        ...
        normalizedNextNodes.forEach((node) => {
          if (previousNodeById.get(node.id) !== node) {
            changedNodeIds.add(node.id);
          }
        });
      }
      ...
      const nextHistory =
        options?.historyMode === 'skip'
          ...
          : {
              past: pushSnapshot(
                state.history.past,
                createNodePatchSnapshot(state.nodes, changedNodeIds, state.edges)
              ),
              future: [],
            };
  ...
  updateNodeSize: (nodeId, size) => {
    ...
    set((state) => {
      ...
      const nextNodes = state.nodes.map((node) => {
        ...
        if (
          Math.round(node.width ?? 0) === nextWidth
          && Math.round(node.height ?? 0) === nextHeight
        ) {
          return node;
        }
  ...
  mergeImageNodesForCompare: (sourceNodeId, targetNodeId) => {
    if (!sourceNodeId.trim() || !targetNodeId.trim()) {
      return { status: 'missingNode' };
    }

    if (sourceNodeId === targetNodeId) {
      return { status: 'sameNode' };
    }

    const state = get();
    const sourceIndex = state.nodes.findIndex((node) => node.id === sourceNodeId);
    const targetIndex = state.nodes.findIndex((node) => node.id === targetNodeId);
    ...
    if (
      (sourceNode.type !== CANVAS_NODE_TYPES.upload && sourceNode.type !== CANVAS_NODE_TYPES.exportImage)
      || (targetNode.type !== CANVAS_NODE_TYPES.upload && targetNode.type !== CANVAS_NODE_TYPES.exportImage)
    ) {
      return { status: 'invalidNode' };
    }

    const overlayImage = buildImageCompareImageSnapshot(sourceNode);
    const baseImage = buildImageCompareImageSnapshot(targetNode);
    const overlayNode = buildImageCompareRestorableNodeSnapshot(sourceNode, sourceIndex);
    const baseNode = buildImageCompareRestorableNodeSnapshot(targetNode, targetIndex);
    if (!overlayImage || !baseImage || !overlayNode || !baseNode) {
      return { status: 'missingImage' };
    }

    const compareNode = canvasNodeFactory.createNode(
      CANVAS_NODE_TYPES.imageCompare,
      {
    ...
    const targetWidth =
      resolveNumericNodeDimension(targetNode.width)
      ?? resolveNumericNodeDimension(targetNode.style?.width);
    const targetHeight =
      resolveNumericNodeDimension(targetNode.height)
      ?? resolveNumericNodeDimension(targetNode.style?.height);
    if (targetWidth !== null) {
      compareNode.width = targetWidth;
    }
    if (targetHeight !== null) {
      compareNode.height = targetHeight;
    }
    ...
    const sizedCompareNode = applyDefaultNodeSize(compareNode, compareNode.data);
    sizedCompareNode.selected = true;

    const compareInsertIndex = state.nodes
      .slice(0, targetIndex)
      .filter((node) => node.id !== sourceNodeId && node.id !== targetNodeId)
      .length;
    const nextNodes: CanvasNode[] = state.nodes
      .filter((node) => node.id !== sourceNodeId && node.id !== targetNodeId)
      .map((node) => ({
        ...node,
        selected: false,
      }));
    nextNodes.splice(compareInsertIndex, 0, sizedCompareNode);
    ...
    const nextNodeDescriptionPanelOpenById = { ...state.nodeDescriptionPanelOpenById };
    delete nextNodeDescriptionPanelOpenById[sourceNodeId];
    delete nextNodeDescriptionPanelOpenById[targetNodeId];

    set({
      nodes: nextNodes,
      edges: nextEdges,
      selectedNodeId: sizedCompareNode.id,
      ...
      nodeDescriptionPanelOpenById: nextNodeDescriptionPanelOpenById,
      history: {
        past: pushSnapshot(state.history.past, createSnapshot(state.nodes, state.edges)),
        future: [],
      },
    ...
    return {
      status: 'success',
      compareNodeId: sizedCompareNode.id,
    };
  },

  splitImageCompareNode: (compareNodeId) => {
    let didSplit = false;

    set((state) => {
      const compareNode = state.nodes.find(
        (node): node is CanvasNode & { data: ImageCompareNodeData } =>
          node.id === compareNodeId && isImageCompareNode(node)
      );
      if (!compareNode || !compareNode.data.mergeMeta) {
        return {};
      }

      const mergeMeta = compareNode.data.mergeMeta;

      const restoredSnapshots = [
        mergeMeta.baseNode,
        mergeMeta.overlayNode,
      ].sort((left, right) => left.originalIndex - right.originalIndex);
      const restoredNodes = restoredSnapshots.map(restoreImageCompareNodeSnapshot);
      if (restoredNodes.length !== 2) {
        return {};
      }
      ...
      restoredSnapshots.forEach((snapshot, index) => {
        const restoredNode = restoredNodes[index];
        const insertIndex = Math.max(0, Math.min(snapshot.originalIndex, nextNodes.length));
        nextNodes.splice(insertIndex, 0, restoredNode);
      });

      const selectedRestoreNodeId = mergeMeta.baseNode.id;
      for (const node of nextNodes) {
        node.selected = node.id === selectedRestoreNodeId;
      }

      const nextEdges = state.edges.filter(
        (edge) => edge.source !== compareNodeId && edge.target !== compareNodeId
      );
      const nextNodeDescriptionPanelOpenById = { ...state.nodeDescriptionPanelOpenById };
      delete nextNodeDescriptionPanelOpenById[compareNodeId];
      didSplit = true;

      return {
        nodes: nextNodes,
        edges: nextEdges,
        selectedNodeId: selectedRestoreNodeId,
        activeToolDialog: resolveActiveToolDialog(state.activeToolDialog, nextNodes),
        activeShotParamsPanelNodeId: resolveActiveShotParamsPanelNodeId(
          state.activeShotParamsPanelNodeId,
          nextNodes
  ...
  fitGroupNodeToChildren: (groupNodeId) => {
    ...
    set((state) => {
      ...
      const nextNodes = fitGroupNodeToChildren(state.nodes, groupNodeId);
      if (nextNodes === state.nodes) {
        return {};
      }

      const changed = nextNodes.some((node, index) => node !== state.nodes[index]);
      if (!changed) {
        return {};
      }
  ...
  setSelectedNodeIds: (nodeIds) => {
    ...
    set((state) => {
      ...
      const nextSelectedNodeId = selectedIdSet.size === 1
        ? nextNodes.find((node) => node.selected)?.id ?? null
        : null;

      if (!didChange && state.selectedNodeId === nextSelectedNodeId) {
        return state;
      }
