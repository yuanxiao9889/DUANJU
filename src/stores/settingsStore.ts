...
import {
  normalizeAdDirectorSkillCategories,
  normalizeAdDirectorSkillTemplates,
...
interface SettingsState {
  ...
  lastMidjourneyAspectRatio: MidjourneyAspectRatio;
  lastMidjourneyVersionPreset: MidjourneyVersionPreset;
  lastMidjourneyRawMode: boolean;
  lastMidjourneyPersonalizationCodes: string[];
  lastMidjourneyAdvancedParams: string;
  ...
  setLastMidjourneyDefaults: (defaults: {
    aspectRatio?: MidjourneyAspectRatio | string;
    versionPreset?: MidjourneyVersionPreset | string;
    rawMode?: boolean;
    personalizationCodes?: string[];
  }) => void;
  setLastMidjourneyAdvancedParams: (advancedParams: string) => void;
  ...
  updateAdDirectorSkillCategory: (
    id: string,
    updates: Partial<Pick<AdDirectorSkillCategory, 'name'>>
  ) => void;
  deleteAdDirectorSkillCategory: (id: string) => void;
  addAdDirectorSkillTemplate: (
    template: Pick<AdDirectorSkillTemplate, 'name' | 'categoryId' | 'profile'>
  ) => string;
  updateAdDirectorSkillTemplate: (
    id: string,
    updates: Partial<Pick<AdDirectorSkillTemplate, 'name' | 'categoryId' | 'profile'>>
  ) => void;
  deleteAdDirectorSkillTemplate: (id: string) => void;
  markAdDirectorSkillTemplateUsed: (id: string) => void;
  importAdDirectorSkillPackageData: (
    data: AdDirectorSkillPackageData
  ) => AdDirectorSkillImportSummary;
...
function normalizeMidjourneyAdvancedParamsValue(input: string | null | undefined): string {
  return typeof input === 'string' ? input.trim() : '';
}

function normalizeMidjourneyAspectRatio(
  input: MidjourneyAspectRatio | string | null | undefined
): MidjourneyAspectRatio {
  return MIDJOURNEY_ASPECT_RATIOS.includes(input as MidjourneyAspectRatio)
    ? (input as MidjourneyAspectRatio)
    : MIDJOURNEY_ASPECT_RATIOS[0];
}

function normalizeMidjourneyVersionPreset(
  input: MidjourneyVersionPreset | string | null | undefined
): MidjourneyVersionPreset {
  return MIDJOURNEY_VERSION_PRESETS.includes(input as MidjourneyVersionPreset)
    ? (input as MidjourneyVersionPreset)
    : MIDJOURNEY_VERSION_PRESETS[0];
}

function normalizeMidjourneyRawMode(input: boolean | null | undefined): boolean {
  return input === true;
}
  ...
  persist(
    (set) => ({
      ...
      setLastMidjourneyDefaults: ({
        ...
        set((state) => ({
          lastMidjourneyAspectRatio: normalizeMidjourneyAspectRatio(
            aspectRatio ?? state.lastMidjourneyAspectRatio
          ),
          lastMidjourneyVersionPreset: normalizeMidjourneyVersionPreset(
            versionPreset ?? state.lastMidjourneyVersionPreset
          ),
          lastMidjourneyRawMode: normalizeMidjourneyRawMode(
            rawMode ?? state.lastMidjourneyRawMode
          ),
          lastMidjourneyPersonalizationCodes: normalizeMjPersonalizationCodes(
            personalizationCodes ?? state.lastMidjourneyPersonalizationCodes
          ),
        })),
      setLastMidjourneyAdvancedParams: (advancedParams) =>
        set({
          lastMidjourneyAdvancedParams: normalizeMidjourneyAdvancedParamsValue(
            advancedParams
          ),
        }),
      ...
      addAdDirectorSkillCategory: ({ name }) => {
        ...
        set((state) => {
          const now = Date.now();
          const nextSortOrder =
            state.adDirectorSkillCategories.reduce(
              (maxOrder, category) => Math.max(maxOrder, category.sortOrder),
              -1
            ) + 1;

          return {
            adDirectorSkillCategories: sortAdDirectorSkillCategories([
      ...
      updateAdDirectorSkillCategory: (id, updates) =>
        set((state) => ({
          adDirectorSkillCategories: sortAdDirectorSkillCategories(
            state.adDirectorSkillCategories.map((category) =>
              category.id === id
                ? {
                    ...category,
                    ...updates,
      ...
      deleteAdDirectorSkillCategory: (id) =>
        set((state) => ({
          adDirectorSkillCategories: state.adDirectorSkillCategories.filter(
            (category) => category.id !== id
          ),
          adDirectorSkillTemplates: sortAdDirectorSkillTemplates(
            state.adDirectorSkillTemplates.map((template) =>
      ...
      addAdDirectorSkillTemplate: (template) => {
        ...
        set((state) => {
          const now = Date.now();
          const nextSortOrder =
            state.adDirectorSkillTemplates.reduce(
              (maxOrder, currentTemplate) =>
                Math.max(maxOrder, currentTemplate.sortOrder),
              -1
            ) + 1;
          const validCategoryIds = new Set(
            state.adDirectorSkillCategories.map((category) => category.id)
          );
          const categoryId =
          ...
          return {
            adDirectorSkillTemplates: sortAdDirectorSkillTemplates([
              ...state.adDirectorSkillTemplates,
              {
                id,
                name: trimmedName,
                categoryId,
      ...
      updateAdDirectorSkillTemplate: (id, updates) =>
        set((state) => ({
          adDirectorSkillTemplates: sortAdDirectorSkillTemplates(
            state.adDirectorSkillTemplates.map((template) => {
              if (template.id !== id) {
              ...
              return {
                ...
                profile: updates.profile ?? template.profile,
                updatedAt: Date.now(),
              };
            })
          ),
        })),
      deleteAdDirectorSkillTemplate: (id) =>
        set((state) => ({
          adDirectorSkillTemplates: state.adDirectorSkillTemplates.filter(
            (template) => template.id !== id
          ),
        })),
      markAdDirectorSkillTemplateUsed: (id) =>
        set((state) => ({
          adDirectorSkillTemplates: sortAdDirectorSkillTemplates(
            state.adDirectorSkillTemplates.map((template) =>
              template.id === id
      ...
      addMjStyleCodePreset: (preset) => {
        ...
        if (!normalizedCode || !normalizedCodeIdentity) {
          return '';
        }
        ...
        set((state) => {
          ...
          const existingPreset = state.mjStyleCodePresets.find(
            (item) =>
              normalizeMjPersonalizationCodeIdentity(item.code) === normalizedCodeIdentity
          );
          if (existingPreset) {
        ...
        set((state) => {
          ...
          const usedCodes = new Set(
            state.mjStyleCodePresets
              ...
              .map((preset) => normalizeMjPersonalizationCodeIdentity(preset.code))
            ...
            state.mjStyleCodePresets.map((preset) => {
              ...
              const nextCodeCandidate =
                typeof updates.code === 'string'
                  ...
                  : preset.code;
              const nextCodeIdentity = normalizeMjPersonalizationCodeIdentity(
                nextCodeCandidate
              );
              const nextCode =
                nextCodeCandidate && nextCodeIdentity && !usedCodes.has(nextCodeIdentity)
                  ? nextCodeCandidate
                  : preset.code;
              const nextImageUrl =
                typeof updates.imageUrl === 'string'
      ...
      importAdDirectorSkillPackageData: (data) => {
        let nextCategories: AdDirectorSkillCategory[] = [];
        let nextTemplates: AdDirectorSkillTemplate[] = [];
        let summary: AdDirectorSkillImportSummary = {
          added: 0,
          updated: 0,
          skipped: 0,
          addedCategories: 0,
        ...
        set((state) => {
          const merged = mergeImportedAdDirectorSkillPackageData({
            currentCategories: state.adDirectorSkillCategories,
            currentTemplates: state.adDirectorSkillTemplates,
            importedCategories: data.categories,
            importedTemplates: data.templates,
          });

          nextCategories = merged.categories;
          nextTemplates = merged.templates;
          summary = merged.summary;

          return {
            adDirectorSkillCategories: nextCategories,
            adDirectorSkillTemplates: nextTemplates,
          };
        });
    ...
    }),
    {
      ...
      migrate: (persistedState: unknown, persistedVersion) => {
        const state = (persistedState ?? {}) as {
          ...
          lastMidjourneyAspectRatio?: MidjourneyAspectRatio | string;
          lastMidjourneyVersionPreset?: MidjourneyVersionPreset | string;
          lastMidjourneyRawMode?: boolean;
          lastMidjourneyPersonalizationCodes?: unknown;
          lastMidjourneyAdvancedParams?: string;
          alibabaTextModel?: string;
          codingModel?: string;
        ...
        const normalizedAdDirectorSkillCategoryIds = new Set(
          normalizedAdDirectorSkillCategories.map((category) => category.id)
        );
        const normalizedAdDirectorSkillTemplates = normalizeAdDirectorSkillTemplates(
          state.adDirectorSkillTemplates,
          normalizedAdDirectorSkillCategoryIds
        );
        ...
        return {
          ...
          lastMidjourneyAspectRatio: normalizeMidjourneyAspectRatio(
            state.lastMidjourneyAspectRatio
          ),
          lastMidjourneyVersionPreset: normalizeMidjourneyVersionPreset(
            state.lastMidjourneyVersionPreset
          ),
          lastMidjourneyRawMode: normalizeMidjourneyRawMode(
            state.lastMidjourneyRawMode
          ),
          lastMidjourneyPersonalizationCodes: normalizeMjPersonalizationCodes(
            state.lastMidjourneyPersonalizationCodes
          ),
          lastMidjourneyAdvancedParams: normalizeMidjourneyAdvancedParamsValue(
            state.lastMidjourneyAdvancedParams
          ),
