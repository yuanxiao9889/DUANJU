...
interface ExtensionsState {
  ...
  discoverLocalExtensionPackages: () => Promise<LoadedExtensionPackage[]>;
...
export const useExtensionsStore = create<ExtensionsState>()(
  persist(
    (set, get) => ({
      ...
      discoverLocalExtensionPackages: async () => {
        const folderPaths = await listLocalExtensionPackages();
        ...
        const loadedPackages = await Promise.all(
          folderPaths.map(async (folderPath) => {
            try {
              return await get().loadExtensionPackage(folderPath);
            } catch (error) {
              console.warn(
                `failed to load discovered extension package: ${folderPath}`,
                error
              );
              return null;
            }
        ...
        return loadedPackages.filter(
          (loadedPackage): loadedPackage is LoadedExtensionPackage => loadedPackage !== null
        );
      },
      ...
      onRehydrateStorage: () => {
        return (state, error) => {
          ...
          void (async () => {
            ...
            try {
              await state?.discoverLocalExtensionPackages();
            } catch (discoverError) {
              console.warn('failed to discover local extension packages', discoverError);
            }

            await state?.syncExtensionRuntimeStates();
          })();
        };
      },
    }
