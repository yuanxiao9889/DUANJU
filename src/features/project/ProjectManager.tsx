...
function ProjectListView({
  ...
  handleLinkClick: (
    ...
    currentLinkedAdProjectId: string | null,
    e: ReactMouseEvent
  ) => void;
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {sortedProjects.map((project) => {
            ...
            const linkedScriptProject = project.linkedScriptProjectId
              ...
              : null;
            const linkedAdProject = project.linkedAdProjectId
              ? adProjects.find((candidate) => candidate.id === project.linkedAdProjectId) ?? null
              : null;
            const activeLinkedSourceType = linkedScriptProject
              ? 'script'
              : linkedAdProject
                ? 'ad'
                : null;
            const activeLinkedSourceName = linkedScriptProject?.name || linkedAdProject?.name || null;
            const linkedStoryboardCount = linkedStoryboardCountByScriptId.get(project.id) ?? 0;
            const linkedStoryboardCountForAd = linkedStoryboardCountByAdId.get(project.id) ?? 0;
                  ...
                  {project.projectType === 'storyboard' ? (
                    <>
                      <p className="flex items-center gap-1.5">
                        <span className="opacity-60">{t('project.linkedSourceLabel')}:</span>
                        <span
                          className={
                        ...
                        >
                          {activeLinkedSourceName
                            ? t(`project.linkedSourceValue.${activeLinkedSourceType}`, {
                                name: activeLinkedSourceName,
                              })
                            : t('project.linkedSourceEmpty')}
                        </span>
                      </p>
                      <p className="flex items-center gap-1.5">
...
function LinkProjectDialog({
  ...
  initialLinkedAdProjectId: string | null;
  onClose: () => void;
  onConfirm: (payload: { type: 'script' | 'ad'; projectId: string | null }) => void;
}) {
  const { t } = useTranslation();
  ...
  const [activeTab, setActiveTab] = useState<'script' | 'ad' | 'clipLibrary'>('script');
  const [selectedScriptProjectId, setSelectedScriptProjectId] = useState(initialLinkedScriptProjectId ?? '');
  const [selectedAdProjectId, setSelectedAdProjectId] = useState(initialLinkedAdProjectId ?? '');
  ...
  useEffect(() => {
    ...
    setSelectedAdProjectId(initialLinkedAdProjectId ?? '');
    setActiveTab(initialLinkedAdProjectId ? 'ad' : 'script');
  ...
  const currentBoundLibrary = storyboardProject.clipLibraryId
    ...
    : null;
  const currentBoundScriptProject = storyboardProject.linkedScriptProjectId
    ? scriptProjects.find((project) => project.id === storyboardProject.linkedScriptProjectId) ?? null
    : null;
  const currentBoundAdProject = storyboardProject.linkedAdProjectId
    ? adProjects.find((project) => project.id === storyboardProject.linkedAdProjectId) ?? null
    : null;
  ...
  const hasBoundScriptProject = Boolean(storyboardProject.linkedScriptProjectId?.trim());
  const hasBoundAdProject = Boolean(storyboardProject.linkedAdProjectId?.trim());
  ...
  const renderScriptTab = () => (
    <div className="space-y-4">
      {currentBoundScriptProject ? (
        <p className="text-xs text-text-muted">
          {t('project.currentLinkedSource')}: {t('project.linkedSourceValue.script', { name: currentBoundScriptProject.name })}
        </p>
      ) : null}
  ...
  const renderAdTab = () => (
    <div className="space-y-4">
      {currentBoundAdProject ? (
        <p className="text-xs text-text-muted">
          {t('project.currentLinkedSource')}: {t('project.linkedSourceValue.ad', { name: currentBoundAdProject.name })}
        </p>
      ) : null}

      <label className="block">
        <div className="mb-2 text-sm font-medium text-text-dark">{t('project.linkAd')}</div>
        <UiSelect
          value={selectedAdProjectId}
          onChange={(event) => setSelectedAdProjectId(event.target.value)}
          className="h-11 w-full rounded-xl text-sm"
        >
          <option value="">{t('project.linkedAdEmpty')}</option>
      ...
      <div className="relative z-10 w-[calc(100vw-2rem)] max-w-lg rounded-2xl border border-border-dark/50 bg-surface-dark/95 p-6 shadow-[0_24px_48px_rgba(0,0,0,0.25)] backdrop-blur-md">
        <div className="mb-5">
          ...
          <p className="mt-2 text-sm text-text-muted">
            {activeTab === 'script'
              ...
              : activeTab === 'ad'
                ? t('project.linkAdDescription', { name: storyboardProject.name })
        ...
        <div className="mt-6 flex gap-3">
          ...
          <UiButton
            ...
            onClick={() => {
              if (activeTab === 'script') {
                onConfirm({ type: 'script', projectId: null });
                return;
              }
              if (activeTab === 'ad') {
                onConfirm({ type: 'ad', projectId: null });
                return;
              }
            ...
            disabled={
              activeTab === 'clipLibrary'
                ? !hasBoundClipLibrary || isSubmittingClipLibrary
                : activeTab === 'script'
                  ? !hasBoundScriptProject
                  : !hasBoundAdProject
            }
          >
            {activeTab === 'clipLibrary'
          ...
          <UiButton
            ...
            onClick={() => {
              if (activeTab === 'script') {
                onConfirm({ type: 'script', projectId: selectedScriptProjectId.trim() || null });
                return;
              }
              if (activeTab === 'ad') {
                onConfirm({ type: 'ad', projectId: selectedAdProjectId.trim() || null });
                return;
              }
...
export function ProjectManager() {
  ...
  const adProjects = useMemo(
    () => projects.filter((project) => project.projectType === 'ad'),
    [projects]
  );
  ...
  const linkedStoryboardCountByAdId = useMemo(() => {
    const nextMap = new Map<string, number>();
    projects.forEach((project) => {
      if (project.projectType !== 'storyboard' || !project.linkedAdProjectId) {
        return;
      }
      nextMap.set(
        project.linkedAdProjectId,
        (nextMap.get(project.linkedAdProjectId) ?? 0) + 1
      );
    });
    return nextMap;
  }, [projects]);
  ...
  const handleLinkClick = (
    ...
    setLinkDialogLinkedAdProjectId(currentLinkedAdProjectId);
  };
  ...
  return (
    <div className="ui-scrollbar h-full min-h-0 w-full overflow-y-auto overflow-x-hidden p-8">
      ...
      <LinkProjectDialog
        ...
        initialLinkedAdProjectId={linkDialogLinkedAdProjectId}
        onClose={() => {
          setLinkDialogProjectId(null);
          setLinkDialogLinkedScriptProjectId(null);
          setLinkDialogLinkedAdProjectId(null);
        }}
        onConfirm={({ type, projectId }) => {
          if (!linkDialogProjectId) {
            return;
          }
          if (type === 'script') {
            void setProjectLinkedScriptProject(linkDialogProjectId, projectId);
          } else {
            void setProjectLinkedAdProject(linkDialogProjectId, projectId);
          }
          setLinkDialogProjectId(null);
          setLinkDialogLinkedScriptProjectId(null);
          setLinkDialogLinkedAdProjectId(null);
