...
interface PrepareNodeVideoFallbackOptions {
  ...
  return await new Promise((resolve, reject) => {
    ...
    const resolveWithMetadata = async () => {
      ...
      try {
        ...
        if (targetTime > 0.01) {
          await waitForVideoSeek(video, clampCaptureTime(video.duration, targetTime));
          await waitForVideoFrameReady(video);
        }
        finishResolve(metadata, captureVideoFrame(video, VIDEO_POSTER_MAX_DIMENSION));
      } catch (error) {
        console.warn('Failed to capture video poster while preparing video asset', {
          fileName: file.name,
          error,
        });
...
export async function prepareNodeVideoFromFile(
  ...
  try {
    const preparedMetadata = await getVideoMetadataAndPoster(file);
    metadata = preparedMetadata.metadata;
    posterDataUrl = preparedMetadata.posterDataUrl;
  } catch (error) {
    if (!fallbackOptions.duration && !fallbackOptions.aspectRatio) {
      throw error;
    }

    console.warn('Failed to extract video metadata from file, using fallback options', {
      fileName: file.name,
      error,
      fallbackOptions,
    });
  ...
  const aspectRatio = metadata
    ...
    : (fallbackOptions.aspectRatio?.trim() || '16:9');
  const duration = metadata
    ? metadata.duration
    : (typeof fallbackOptions.duration === 'number' && Number.isFinite(fallbackOptions.duration)
      ? Math.max(0, fallbackOptions.duration)
      : 0);
...
async function getVideoMetadataFromSource(source: string): Promise<VideoMetadata> {
...
export async function prepareNodeVideoFromSource(source: string): Promise<PreparedVideo> {
  ...
  const captureTime =
    Number.isFinite(metadata.duration) && metadata.duration > 0.18
      ? Math.min(0.12, Math.max(metadata.duration / 10, 0.04))
      : 0;
  const posterDataUrl = await captureVideoFrameFromSource(
    persistedVideoPath,
    captureTime,
  ...
  return {
    videoUrl: persistedVideoPath,
    previewImageUrl,
    aspectRatio: reduceAspectRatio(metadata.width, metadata.height),
