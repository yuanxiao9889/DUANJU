...
export interface GeneratedTtsAudioAsset {
  audioUrl: string;
...
interface TtsAudioQueueJob {
  audioNodeId: string;
  sourceNodeId: string;
  run: () => Promise<GeneratedTtsAudioAsset>;
}
...
let activeJob: TtsAudioQueueJob | null = null;
const pendingJobs: TtsAudioQueueJob[] = [];
let progressTimerId: number | null = null;
let progressStageTimerId: number | null = null;

function resolveErrorMessage(error: unknown): string {
  return resolveErrorContent(error, i18n.t('node.audioNode.generationFailedTitle')).message;
}
...
function startRunningProgress(audioNodeId: string): void {
  ...
  updateAudioNodeState(audioNodeId, {
    ...
    queuePosition: null,
    statusText: i18n.t('node.audioNode.statusPreparing'),
    lastError: null,
  });
  ...
  progressStageTimerId = window.setTimeout(() => {
    ...
    updateAudioNodeState(audioNodeId, {
      ...
      statusText: i18n.t('node.audioNode.statusRendering'),
    });
  }, PREPARING_DELAY_MS);
...
export function enqueueTtsAudioGeneration(job: TtsAudioQueueJob): void {
  pendingJobs.push(job);
  syncQueuedNodeStates();
  void processNextJob();
}

export const enqueueQwenTtsAudioGeneration = enqueueTtsAudioGeneration;
