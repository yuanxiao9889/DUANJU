...
import {
  ...
  findStoryboardOopiiBuiltinModel,
...
import { COMFLY_GPT_IMAGE_2_MODEL_ID } from './image/comfly/gptImage2';
import { GRSAI_GPT_IMAGE_2_MODEL_ID } from './image/grsai/gptImage2';
import { GRSAI_NANO_BANANA_2_MODEL_ID } from './image/grsai/nanoBanana2';
import { GRSAI_NANO_BANANA_PRO_MODEL_ID } from './image/grsai/nanoBananaPro';
...
import { ZHENZHEN_GPT_IMAGE_2_MODEL_ID } from './image/zhenzhen/gptImage2';
...
> = {
  oopii: STORYBOARD_OOPII_BUILTIN_MODELS.map((model) => ({
    modelId: model.id,
  ...
  comfly: [
    ...
    { modelId: COMFLY_GPT_IMAGE_2_MODEL_ID, label: 'gpt-image-2', source: 'builtin' },
  ],
  zhenzhen: [
    ...
    { modelId: ZHENZHEN_GPT_IMAGE_2_MODEL_ID, label: 'gpt-image-2', source: 'builtin' },
  ],
  bltcy: [
    ...
    { modelId: BLTCY_GPT_IMAGE_2_MODEL_ID, label: 'gpt-image-2', source: 'builtin' },
  ],
...
function createOopiiCustomStoryboardImageModel(
  ...
  const resolvedConfig = resolveStoryboardOopiiModelConfigForModel(
    toStoryboardProviderModelId(STORYBOARD_OOPII_PROVIDER_ID, entry.modelId),
    { [STORYBOARD_OOPII_PROVIDER_ID]: [entry] }
  );
  const legacyModel = createStoryboardOopiiImageModel(resolvedConfig);
  if (!legacyModel) {
    return null;
  }

  const matchedBuiltinModel = findStoryboardOopiiBuiltinModel(entry.modelId);
  return {
    ...legacyModel,
    displayName:
