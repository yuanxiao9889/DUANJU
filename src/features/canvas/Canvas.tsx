...
interface AlignmentDragSession {
  ...
  otherNodeBounds: NodeBounds[] | null;
  hasMovedEnough: boolean;
  lastSnapOffset: CanvasPoint;
  lastSnapPosition: CanvasPoint | null;
}
...
function buildScriptWelcomePresenceSignature(nodes: CanvasNode[]): string {
  ...
  for (const node of nodes) {
    if (node.type === CANVAS_NODE_TYPES.scriptChapter) {
      hasChapters = true;
    } else if (node.type === CANVAS_NODE_TYPES.scriptRoot) {
      hasRoot = true;
    }
...
function hasAnyConnectedEdges(nodeId: string, edges: CanvasEdge[]): boolean {
...
function resolveImageCompareNumericAspectRatio(node: CanvasNode): number | null {
  ...
  const aspectRatio = parseAspectRatio(imageSource.aspectRatio);
...
function findImageCompareDropTarget(
  ...
  const candidateTargets = allNodes
    ...
    .map((candidate) => {
      ...
      const containsCenter =
        sourceCenter.x >= candidatePosition.x &&
        sourceCenter.x <= candidatePosition.x + candidateSize.width &&
        sourceCenter.y >= candidatePosition.y &&
        sourceCenter.y <= candidatePosition.y + candidateSize.height;

      return containsCenter
        ? {
            node: candidate,
            area: candidateSize.width * candidateSize.height,
          }
        : null;
    })
...
function areAlignmentGuidesEqual(
  ...
  if (previousGuides.length !== nextGuides.length) {
    return false;
  }
...
function collectClipboardFileSources(sourcePaths: string[]): ClipboardFileSource[] {
  ...
  for (const rawPath of sourcePaths) {
    ...
    const kind = resolveClipboardFileKind(normalizedPath);
    if (!kind) {
      continue;
    }
    ...
    items.push({
      path: normalizedPath,
      kind,
      fileName: resolveClipboardSourceFileName(normalizedPath),
    });
  }
...
export function Canvas() {
  ...
  const viewportStoreRef = useRef<Viewport>(DEFAULT_VIEWPORT);
  ...
  const flushPendingAlignment = useCallback(() => {
    ...
    const pendingNode = pendingAlignmentNodeRef.current;
    pendingAlignmentNodeRef.current = null;
    if (!pendingNode || !enableNodeAlignment || altDragCopyRef.current) {
      return;
    }
    ...
    if (!alignmentSession.otherNodeBounds) {
      alignmentSession.otherNodeBounds = currentNodes
        .filter(
          (candidate) =>
    ...
    if (Math.abs(snapOffset.x) <= 0.01 && Math.abs(snapOffset.y) <= 0.01) {
      alignmentSession.lastSnapOffset = { x: 0, y: 0 };
      alignmentSession.lastSnapPosition = null;
      return;
    }
  ...
  const scriptWelcomePresenceSignature = useMemo(() => {
    ...
    const signature = buildScriptWelcomePresenceSignature(nodes);
  ...
  const pendingGenerationNodeIdSignature = useMemo(() => {
    ...
    const signature = buildPendingGenerationNodeIdSignature(nodes);
    pendingGenerationNodeIdSignatureRef.current = signature;
    return signature;
  }, [dragHistorySnapshot, nodes]);
  ...
  const episodeCatalogItems = useMemo(() => {
  ...
  const episodeCatalogSceneNode = useMemo(() => {
    if (!episodeCatalogSceneNodeId) {
      return null;
    }
  ...
  useEffect(() => {
    ...
    const selectedScriptData = selectedShootingScriptNode.data as {
      sourceSceneNodeId?: string;
      sourceEpisodeId?: string;
    };
    if (activeScriptNodeId === selectedShootingScriptNode.id) {
      return;
    }

    focusShootingScript(selectedShootingScriptNode.id, {
      sceneNodeId: selectedScriptData.sourceSceneNodeId ?? null,
      episodeId: selectedScriptData.sourceEpisodeId ?? null,
      cell: null,
    });
  }, [
    activeScriptNodeId,
  ...
  useEffect(() => {
    ...
    const applyPreparedImageToSelectedUploadNode = (
      ...
      updateNodeData(selectedUploadNodeId, {
        imageUrl: imageData.imageUrl,
        previewImageUrl: imageData.previewImageUrl,
        thumbnailUrl: imageData.thumbnailImageUrl,
        aspectRatio: imageData.aspectRatio,
    ...
    const createNodesFromClipboardFilePaths = async (sourcePaths: string[]): Promise<boolean> => {
      ...
      const clipboardSources = collectClipboardFileSources(sourcePaths);
      if (clipboardSources.length === 0) {
        return true;
      }
      ...
      const basePosition = resolveClipboardInsertPosition();
      const nodeOffset = 30;
      let lastCreatedNodeId: string | null = null;

      for (let index = 0; index < clipboardSources.length; index += 1) {
        const item = clipboardSources[index];
        const position = {
          x: basePosition.x + (index % 4) * nodeOffset,
          y: basePosition.y + Math.floor(index / 4) * nodeOffset,
        };
        ...
        try {
          ...
          const audioData = await prepareNodeAudio(item.path);
          lastCreatedNodeId = addNode(CANVAS_NODE_TYPES.audio, position, {
      ...
      if (lastCreatedNodeId) {
        setSelectedNode(lastCreatedNodeId);
      }
    ...
    const handlePaste = (event: ClipboardEvent) => {
      ...
      pendingPasteHandledRef.current = null;
      ...
      if (imageFile || isTauri()) {
        event.preventDefault();
      }

      const pendingPastePromise = (async () => {
        if (isTauri()) {
          const clipboardFilePaths = await readSystemClipboardFilePaths();
          if (clipboardFilePaths.length > 0) {
            return await createNodesFromClipboardFilePaths(clipboardFilePaths);
          }
        }
        ...
        await createUploadNodeFromClipboardImage(imageFile);
        return true;
      })()
        .then((handled) => {
          pasteImageHandledRef.current = handled;
          return handled;
        })
        .catch((error) => {
      ...
      pendingPasteHandledRef.current = pendingPastePromise;
      void pendingPastePromise.finally(() => {
        if (pendingPasteHandledRef.current === pendingPastePromise) {
          pendingPasteHandledRef.current = null;
        }
      });
    };
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (isPaste) {
        ...
        window.setTimeout(() => {
          ...
          if (pendingPasteHandled) {
            void pendingPasteHandled.then((handled) => {
              pasteImageHandledRef.current = false;
              if (handled) {
                return;
              }
  ...
  const handleNodeDrag = useCallback(
    (_event: ReactMouseEvent, node: CanvasNode) => {
      ...
      if (enableNodeAlignment && !altCopyState) {
        scheduleAlignmentDuringDrag(node);
      }
