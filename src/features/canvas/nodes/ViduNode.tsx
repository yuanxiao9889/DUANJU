...
function findViduPromptReferenceTokens(
  prompt: string,
  maxReferenceCount: number,
): ViduPromptReferenceToken[] {
...
function findViduPromptTokenRanges(
  prompt: string,
  maxReferenceCount: number,
): ViduPromptTokenRange[] {
  return findViduPromptReferenceTokens(prompt, maxReferenceCount).map(
    (token) => ({
      start: token.start,
      end: token.end,
      blockStart:
...
function renderPromptWithHighlights(
  ...
  let lastIndex = 0;
  const referenceTokens = findViduPromptReferenceTokens(
    prompt,
    maxReferenceCount,
  );
...
function areImageAspectRatiosCompatible(
  ...
  const ratioDelta =
    Math.min(ratios[0], ratios[1]) / Math.max(ratios[0], ratios[1]);
...
export const ViduNode = memo(({ id, data, selected, width }: ViduNodeProps) => {
  ...
  const selectedModelId = normalizeViduModelIdForInputMode(
    data.modelId,
    selectedInputMode,
  );
  ...
  const applyPromptAndRestoreCursor = useCallback(
    (prompt: string, cursor: number) => {
      ...
      schedulePromptSelectionRestore(
        Math.max(0, Math.min(cursor, prompt.length)),
      );
    },
  ...
  const validateGenerationRequest = useCallback((): string | null => {
    ...
    if (connectedAudioReferences.length > 0) {
      ...
      if (
        !/^https?:\/\//i.test(
          connectedAudioReferences[0]?.audioUrl?.trim() ?? "",
        )
      ) {
    ...
    if (
      selectedInputMode === "firstFrame" &&
      (connectedImages.length !== 1 || connectedVideos.length > 0)
    ) {
      return t("node.vidu.firstFrameRequiresOneImage");
    }
    if (
      selectedInputMode === "firstLastFrame" &&
      (connectedImages.length !== 2 || connectedVideos.length > 0)
    ) {
    ...
    if (selectedInputMode === "reference") {
      if (
        ...
        connectedImages.length > VIDU_MAX_REFERENCE_IMAGE_COUNT
      ) {
        return t("node.vidu.referenceImageLimit", {
          count: VIDU_MAX_REFERENCE_IMAGE_COUNT,
        });
      }
      if (connectedVideos.length > VIDU_MAX_REFERENCE_VIDEO_COUNT) {
        return t("node.vidu.referenceVideoLimit", {
          count: VIDU_MAX_REFERENCE_VIDEO_COUNT,
        });
      }
      if (
        connectedVideos.length > 0 &&
        selectedModelId !== VIDU_REFERENCE_VIDEO_MODEL_ID
      ) {
  ...
  const validateStartEndImageRatios = useCallback(async (): Promise<
    string | null
  > => {
    if (
      selectedInputMode !== "firstLastFrame" ||
  ...
  const handleGenerate = useCallback(async () => {
    ...
    const validationError =
      validateGenerationRequest() ?? (await validateStartEndImageRatios());
    if (validationError) {
      updateCurrentNodeData({
        isSubmitting: false,
    ...
    try {
      ...
      const submitResponse = await submitViduVideoTask({
        ...
        referenceAudioSources: connectedAudioReferences.map(
          (item) => item.audioUrl,
        ),
      });
            ...
            <div className="ui-scrollbar min-w-0 flex-1 overflow-x-auto overflow-y-hidden">
              <div className="flex w-max min-w-full items-center gap-1.5 pr-1">
                <FixedControlChip
                  ...
                  onChange={(value) =>
                    updateCurrentNodeData({ modelId: value, lastError: null })
                  }
                />
                <FixedControlChip
                  ...
                  onChange={(value) =>
                    updateCurrentNodeData({ inputMode: value, lastError: null })
                  }
                />
                {supportsSelectableAspectRatio ? (
                  <FixedControlChip
                    ...
                    onChange={(value) =>
                      updateCurrentNodeData({ aspectRatio: value })
                    }
                  />
                ) : null}
                <FixedControlChip
                  ...
                  onChange={(value) =>
                    updateCurrentNodeData({ durationSeconds: value })
                  }
                />
                <FixedControlChip
                  ...
                  onChange={(value) =>
                    updateCurrentNodeData({ resolution: value })
                  }
                />
                <FixedControlChip
                ...
                <UiChipButton
                  ...
                  aria-label={
                    isOptimizingPrompt
                      ? t("node.vidu.optimizingPrompt")
                      : t("node.vidu.optimizePrompt")
                  }
                  title={
                    isOptimizingPrompt
                      ? t("node.vidu.optimizingPrompt")
