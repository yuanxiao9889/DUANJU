  ...
  return (
    ...
    >
      <div className="flex h-full flex-col">
        ...
        <div className="mt-auto pt-2.5 text-[10px] leading-4 text-text-muted">
          {actionLabel}
        </div>
      </div>
    </button>
...
export const QwenTtsSavedVoiceNode = memo(
  ({ id, data, selected }: QwenTtsSavedVoiceNodeProps) => {
    ...
    const [presetPreviewError, setPresetPreviewError] = useState<string | null>(
      null,
    );
    ...
    const activeExtensionPackage =
      readyExtensionPackage ?? qwenTtsExtensionState.pendingPackage;
    ...
    const supportsMp3Output =
      activeExtensionPackage?.id === QWEN_TTS_COMPLETE_EXTENSION_ID;
    ...
    const selectedPresetAsset = useMemo(() => {
      const presetAssetId =
        typeof data.presetAssetId === "string" ? data.presetAssetId.trim() : "";
      if (!presetAssetId) {
        return null;
      }
    ...
    const manualReferenceAudioPath =
      legacyReferenceAudioData?.audioUrl?.trim() ?? "";
    const manualReferenceAudioName =
      legacyReferenceAudioData?.audioFileName?.trim() ?? "";
    const manualReferenceAudioDuration = formatAudioDuration(
      legacyReferenceAudioData?.duration,
    );
    const isUsingPreset = Boolean(selectedPresetAsset);
    const referenceAudioPath =
      selectedPresetAsset?.sourcePath?.trim() ?? manualReferenceAudioPath ?? "";
    const referenceAudioName =
      selectedPresetAsset?.name?.trim() ?? manualReferenceAudioName ?? "";
    ...
    const referenceTranscript =
      ...
      (typeof data.referenceTranscript === "string"
    ...
    const progressValue =
      typeof data.generationProgress === "number"
    ...
    const runtimeHint = isExtensionReady
      ...
      : isExtensionStarting
        ? (currentRuntimeStep?.description ??
          t("node.qwenTts.runtimeStateStarting"))
    ...
    const selectedPresetDescription =
      selectedPresetAsset?.description?.trim() ?? "";
    const selectedPresetLibraryName = useMemo(
      () =>
        selectedPresetAsset
          ? (assetLibraries.find(
              (library) => library.id === selectedPresetAsset.libraryId,
            )?.name ?? "")
          : "",
    ...
    const availablePresetCount = useMemo(
      () =>
        availablePresetGroups.reduce(
          (count, library) => count + library.items.length,
          0,
        ),
    ...
    const pendingAudioTaskCountSelector = useMemo(
      () => (state: ReturnType<typeof useCanvasStore.getState>) => {
        ...
        return state.edges
          ...
          .filter((audioData) => {
            if (
              audioData.generationSource !== "ttsSavedVoice" ||
              audioData.audioUrl
            ) {
              return false;
            }
            return (
              ...
              typeof audioData.queuePosition === "number"
            );
          }).length;
      },
      [id],
    ...
    const handleExtractVoice = useCallback(async (): Promise<{
      ...
      try {
        ...
        const savedPrompt = await createQwenTtsSavedVoicePrompt(
          ...
          await updateAssetItem({
            ...
            metadata: {
              ...
              voicePreset: {
                ...
                sourceGeneration:
                  selectedPresetMetadata?.sourceGeneration ?? "ttsVoiceDesign",
        ...
        updateNodeData(
          ...
          {
            presetAssetId:
              selectedPresetAsset?.id ?? data.presetAssetId ?? null,
            voiceName: suggestedVoiceName,
            promptFile: savedPrompt.promptFile,
            promptLabel: savedPrompt.promptLabel,
            isExtracting: false,
        ...
        updateErrorState(
          resolveErrorContent(error, t("node.qwenTts.savedVoice.extractFailed"))
            .message,
        );
        return null;
      } finally {
        ...
        >
          <NodeHeader
            ...
            titleText={resolveNodeDisplayName(
              CANVAS_NODE_TYPES.ttsSavedVoice,
              data,
            )}
            ...
            <div className="rounded-2xl border border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(56,189,248,0.16),transparent_42%),linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))] p-3">
              ...
              <div className="mt-3 grid grid-cols-3 gap-2">
                ...
                <div className="rounded-xl border border-white/10 bg-black/10 px-3 py-2">
                  ...
                  <div className="text-[11px] text-text-muted">
                    {t("node.qwenTts.connectedTextStats", {
                      count: connectedLineCount,
                    })}
                  </div>
                </div>
              </div>
            ...
            {isOverviewRender ? (
              <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-3 text-xs leading-5 text-text-muted">
                ...
                <div className="mt-2 line-clamp-4 whitespace-pre-wrap">
                  {referenceTranscriptTrimmed ||
                    t("node.qwenTts.savedVoice.noReferenceTranscript")}
                </div>
              </div>
            ) : (
              <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-3">
                ...
                <div className="space-y-3">
                      ...
                      <div className="mb-1 flex items-center justify-between gap-2">
                        ...
                        >
                          {isUsingPreset
                            ...
                            : manualReferenceAudioPath
                              ? t(
                                  "node.qwenTts.savedVoice.referenceSourceManual",
                                )
                              : t("node.qwenTts.savedVoice.referenceMissing")}
                        </span>
                      </div>
                  ...
                  <label className="block">
                    ...
                    {isUsingPreset ? (
                      ...
                      <textarea
                        ...
                        placeholder={t(
                          "node.qwenTts.savedVoice.referenceTranscriptPlaceholder",
                        )}
                  ...
                  <div className="rounded-xl border border-white/10 bg-black/10 px-3 py-3">
                    <div className="mb-2 flex items-start justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-[11px] leading-4 text-text-muted">
                          {t(
                            "node.qwenTts.savedVoice.presetOverridesManualHint",
                          )}
                        </div>
                      </div>
                      {selectedPresetAsset ? (
                    ...
                    >
                      <option value="">
                        {t("node.qwenTts.savedVoice.selectPresetPlaceholder")}
                      </option>
            ...
            {!isOverviewRender ? (
              <div className="grid grid-cols-3 gap-2">
                <SummaryActionCard
                  ...
                  statusLabel={
                    hasReuseOverrides ? t("node.qwenTts.customizedShort") : null
                  }
                ...
                <SummaryActionCard
                  ...
                  actionLabel={t("node.qwenTts.openSettingsShort")}
                  statusLabel={
                    hasPauseOverrides ? t("node.qwenTts.customizedShort") : null
                  }
            ...
            {data.isExtracting ? (
              <div className="space-y-2 rounded-xl border border-accent/20 bg-accent/10 px-3 py-2.5">
                ...
                <div className="text-xs text-text-muted">
                  {data.statusText ??
                    t("node.qwenTts.savedVoice.statusExtracting")}
                </div>
              </div>
            ) : null}
        ...
        >
          <div className="grid grid-cols-2 gap-3">
            ...
            <SliderField
              ...
              helperText={t(
                "node.qwenTts.parameterDescriptions.repetitionPenalty",
              )}
              min={1}
              max={2}
              step={0.05}
