...
import type {
  SummaryExpandContinuityContext,
  SummaryExpandRequest,
...
function splitGeneratedChapterHtmlIntoScenes(
  ...
  const commitCurrentScene = () => {
    ...
    const fallbackScene =
      fallbackScenes[sceneDrafts.length] ??
      createDefaultSceneCard(sceneDrafts.length);
    const summarySource =
      draftHtml.replace(/^<h[23][^>]*>.*?<\/h[23]>/i, "").trim() || draftHtml;
    sceneDrafts.push({
      title: currentTitle.trim() || fallbackScene.title,
      draftHtml,
      summary: summarizeGeneratedSceneDraft(
        summarySource,
  ...
  elements.forEach((element) => {
    ...
    if (currentParts.length === 0) {
      currentTitle =
        fallbackScenes[sceneDrafts.length]?.title ||
        `Scene ${sceneDrafts.length + 1}`;
    }
...
const SCRIPT_CHAPTER_GRAPH_NODE_TYPES = [
  CANVAS_NODE_TYPES.scriptChapter,
] as const;
const SCRIPT_STORY_NOTE_GRAPH_NODE_TYPES = [
  CANVAS_NODE_TYPES.scriptStoryNote,
] as const;
export const SCRIPT_CHAPTER_NODE_DRAG_HANDLE_CLASS =
...
export const ScriptChapterNode = memo(
  ({ id, data, selected, width, height }: ScriptChapterNodeProps) => {
    const { t } = useTranslation();
    const isOverviewRender = useIsOverviewCanvasRender();
    const updateNodeData = useCanvasStore((state) => state.updateNodeData);
    const setSelectedNode = useCanvasStore((state) => state.setSelectedNode);
    const scriptSceneNodes = useCanvasNodesByTypes(
      SCRIPT_SCENE_GRAPH_NODE_TYPES,
    );
    const scriptChapterNodes = useCanvasNodesByTypes(
      SCRIPT_CHAPTER_GRAPH_NODE_TYPES,
    );
    const scriptRootNode = useCanvasFirstNodeByType(
      CANVAS_NODE_TYPES.scriptRoot,
    );
    const scriptStoryNoteNodes = useCanvasNodesByTypes(
      SCRIPT_STORY_NOTE_GRAPH_NODE_TYPES,
    );
    ...
    const focusChapterScene = useScriptEditorStore(
      (state) => state.focusChapterScene,
    );
    ...
    const [aiDialogMode, setAiDialogMode] = useState<
      "expandFromSummary" | "expandFromMerged" | null
    >(null);
    ...
    const sceneNodeBySceneId = useMemo(() => {
      const nextMap = new Map<
        string,
        { id: string; data: ScriptSceneNodeData }
      >();
    ...
    const hasMergedBranches = Boolean(
      data.mergedFromBranches && data.mergedFromBranches.length > 0,
    );
    const isMergePoint =
      data.isMergePoint ||
      (hasMergedBranches && (data.mergedFromBranches?.length ?? 0) >= 2);
    ...
    const chapterContinuityContext =
      useMemo<SummaryExpandContinuityContext | null>(() => {
        const sortedChapters = getSortedScriptChapterNodes(scriptChapterNodes);
        const currentIndex = sortedChapters.findIndex((node) => node.id === id);
        if (currentIndex < 0) {
          return null;
        ...
        const relatedMemories = (baseContext?.relevantMemories ?? [])
          .filter(
            (memory) =>
              !primaryHandoff ||
              memory.label !== primaryHandoff.label ||
              memory.summary !== primaryHandoff.summary,
          )
          .slice(0, 2);
        const guardrails = dedupeTrimmedStrings(
          baseContext?.guardrails ?? [],
          6,
        );
        ...
        return {
          guardrails,
          relevantMemories: primaryHandoff
            ? [primaryHandoff, ...relatedMemories]
            : relatedMemories,
        };
      }, [
        id,
    ...
    const characterNotes = useMemo(
      () =>
        collectConnectedScriptCharacterNotes(
          id,
          incomingCharacterNodes,
          incomingCharacterEdges,
        ),
      [id, incomingCharacterEdges, incomingCharacterNodes],
    );
    const summaryExpandContext = useMemo<
      Omit<SummaryExpandRequest, "instruction">
    >(
      () => ({
        ...
        storyRoot: storyRootData
          ? {
              ...
              characterLibraryNames: storyRootData.assetLibraryCharacters.map(
                (item) => item.name,
              ),
              locationLibraryNames: storyRootData.assetLibraryLocations.map(
                (item) => item.name,
              ),
              itemLibraryNames: storyRootData.assetLibraryItems.map(
                (item) => item.name,
              ),
            }
          : null,
    ...
    const scheduleHandoffMemoryRefresh = useCallback(
      (sceneId: string, expectedDraftHtml: string) => {
        void (async () => {
          try {
            const initialState = useCanvasStore.getState();
            const initialNode = initialState.nodes.find(
              (node) =>
                node.id === id && node.type === CANVAS_NODE_TYPES.scriptChapter,
            );
            if (!initialNode) {
              return;
            }

            const initialChapter = normalizeScriptChapterNodeData(
              initialNode.data as ScriptChapterNodeData,
            );
            const initialScenes = normalizeSceneCards(
              initialChapter.scenes,
              initialChapter.content,
            );
            const targetScene = initialScenes.find(
              (scene) => scene.id === sceneId,
            );
            if (
              !targetScene ||
              targetScene.draftHtml.trim() !== expectedDraftHtml.trim()
            ) {
              return;
            }
            ...
            const latestNode = latestState.nodes.find(
              (node) =>
                node.id === id && node.type === CANVAS_NODE_TYPES.scriptChapter,
            );
            if (!latestNode) {
            ...
            const latestChapter = normalizeScriptChapterNodeData(
              latestNode.data as ScriptChapterNodeData,
            );
            const latestScenes = normalizeSceneCards(
              latestChapter.scenes,
              latestChapter.content,
            );
            const latestScene = latestScenes.find(
              (scene) => scene.id === sceneId,
            );
            if (
              !latestScene ||
              latestScene.draftHtml.trim() !== expectedDraftHtml.trim()
            ) {
              return;
            }
            ...
            const nextScenes = latestScenes.map((scene) =>
              scene.id === sceneId
                ? {
                    ...
                    sourceDraftHtml: scene.draftHtml.trim()
                      ? scene.draftHtml
                      : scene.sourceDraftHtml,
                    sourceDraftLabel:
                      scene.sourceDraftLabel?.trim() ||
                      t("script.sceneWorkbench.generatedSourceLabel"),
                  }
                : scene,
            );
            latestState.updateNodeData(
              id,
              {
                scenes: nextScenes,
                sceneHeadings: nextScenes
            ...
            console.warn(
              "[ScriptChapterNode] Failed to refresh handoff memory",
              error,
            );
          }
        })();
    ...
    const handleAiConfirm = useCallback(
      (result: string) => {
        if (
          aiDialogMode !== "expandFromSummary" &&
          aiDialogMode !== "expandFromMerged"
        ) {
          return;
        }

        const generatedScenes = splitGeneratedChapterHtmlIntoScenes(
          result,
          scenes,
        );
        if (generatedScenes.length > 0) {
          const nextScenes = [
            ...generatedScenes.map((generatedScene, index) => {
              const fallbackScene =
                scenes[index] ?? createDefaultSceneCard(index);
              return {
                ...fallbackScene,
                order: index,
                ...
                sourceDraftHtml:
                  generatedScene.draftHtml || fallbackScene.sourceDraftHtml,
                sourceDraftLabel:
                  fallbackScene.sourceDraftLabel?.trim() ||
                  t("script.sceneWorkbench.generatedSourceLabel"),
                continuitySummary: "",
                continuityFacts: [],
                continuityOpenLoops: [],
                continuityUpdatedAt: null,
                status: generatedScene.draftHtml.trim()
                  ? "drafting"
          ...
          if (lastGeneratedScene?.draftHtml.trim()) {
            scheduleHandoffMemoryRefresh(
              lastGeneratedScene.id,
              lastGeneratedScene.draftHtml,
            );
          }
        } else {
      ...
      <>
        ...
        >
                ...
                {hasMergedBranches ? (
                  <div className="mt-1 flex flex-wrap items-center gap-1">
                    ...
                    {mergedBranchContents
                      .filter((branch) => branch.branchLabel)
                ...
                <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    <div className="mt-1 text-sm font-medium text-text-dark">
                      {t("script.sceneStudio.sceneCount", {
                        count: scenes.length,
                      })}
                    </div>
                  </div>
                        ...
                        <>
                          {isMergePoint ? (
                            <button
                              ...
                              title={t(
                                "script.chapterCatalog.expandFromMerged",
                              )}
                            >
                ...
                {isOverviewRender ? (
                  ...
                  >
                    {scenes.map((scene) => {
                      ...
                      const isFocusedScene =
                        activeChapterId === id &&
                        activeChapterSceneId === scene.id;
                      const isActive =
                        isFocusedScene || activeSceneNodeId === sceneNode?.id;
                          ...
                          <div className="flex items-center justify-between gap-2">
                            <div className="min-w-0">
                              ...
                              <div className="mt-1 text-[11px] text-text-muted">
                                {t("script.sceneStudio.sceneLabel", {
          ...
          <CanvasHandle
            ...
            className="!top-1/2 !rounded-full !border-2 !border-surface-dark !bg-accent"
          />
          <NodeResizeHandle
        ...
        {aiDialogMode && !isOverviewRender ? (
          <AiWriterDialog
            ...
            summaryExpandContext={
              aiDialogMode === "expandFromSummary" ? summaryExpandContext : null
            }
            mergedBranchContents={
              hasMergedBranches ? mergedBranchContents : undefined
            }
