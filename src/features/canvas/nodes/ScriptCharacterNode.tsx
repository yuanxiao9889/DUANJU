...
export const ScriptCharacterNode = memo(
  ({ id, data, selected, width, height }: ScriptCharacterNodeProps) => {
    ...
    const hasContent = Boolean(
      data.description || data.personality || data.appearance,
    );
    ...
    const persistCharacterAssetLibrary = useCallback(
      (nextData: {
        name: string;
        description: string;
        personality: string;
    ...
    const { isOptimizing, handleOptimize } = useScriptAssetOptimization({
      ...
      optimize: (nodes) =>
        optimizeScriptCharacterFields(getOptimizationSource(), nodes),
      ...
      <ScriptNodeCard
        ...
        headerActions={
          <button
            ...
            title={
              isOptimizing
                ? t("scriptNodes.common.optimizing")
                : t("scriptNodes.common.optimize")
            }
        ...
        overlayContent={
          <>
            <CanvasHandle
              ...
              className="!rounded-full !border-2 !border-surface-dark !bg-accent"
            />
            <NodeResizeHandle
              minWidth={MIN_WIDTH}
      ...
      >
        {isEditing ? (
          <div className={`${SCRIPT_NODE_SCROLL_AREA_CLASS} space-y-3`}>
            <div>
              <label className={SCRIPT_NODE_LABEL_CLASS}>
                {t("scriptNodes.character.name")}
              </label>
              <input
                type="text"
                value={editData.name}
          ...
          <div
            className={`${SCRIPT_NODE_SCROLL_AREA_CLASS} space-y-3 text-sm text-text-dark/88`}
          >
            {data.description ? (
