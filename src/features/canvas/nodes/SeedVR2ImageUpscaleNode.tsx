...
function localizeSeedvr2ImageError(
  errorMessage: string,
...
export const SeedVR2ImageUpscaleNode = memo(
  ({ id, data, selected }: SeedVR2ImageUpscaleNodeProps) => {
    ...
    const runtimeMessage = useMemo(() => {
      ...
      if (extensionRuntime?.status === "error") {
        return (
          ...
          t("node.seedvr2ImageUpscale.runtimeErrored")
        );
      }
    ...
    const handleResolutionChange = (value: string) => {
      ...
      if (
        !SEEDVR2_IMAGE_TARGET_RESOLUTIONS.includes(
          nextResolution as Seedvr2ImageTargetResolution,
        )
      ) {
        return;
      }
      ...
      updateNodeData(id, {
        targetResolution: nextResolution as Seedvr2ImageTargetResolution,
      });
    };
    ...
    const handleExecute = async () => {
      ...
      try {
        ...
        const preparedImage = await upscaleImageWithSeedvr2(
        ...
        const errorMessage = localizeSeedvr2ImageError(
          resolveErrorMessage(
            error,
            t("node.seedvr2ImageUpscale.processFailed"),
          ),
          t,
        );
        updateNodeData(
      ...
      >
        <NodeHeader
          ...
          titleText={resolveNodeDisplayName(
            CANVAS_NODE_TYPES.seedvr2ImageUpscale,
            data,
          )}
          editable
            ...
            <div className="flex items-start gap-3">
              {inputState.previewImageUrl ? (
                <CanvasNodeImage
                  ...
                  fallbackSrc={
                    "imageUrl" in inputState ? inputState.imageUrl : null
                  }
            ...
            >
              {SEEDVR2_IMAGE_TARGET_RESOLUTIONS.map((resolution) => (
                <option key={resolution} value={resolution}>
                  {t("node.seedvr2ImageUpscale.targetResolutionOption", {
                    value: resolution,
                  })}
                </option>
              ))}
