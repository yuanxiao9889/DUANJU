...
function normalizeLayerOrdering(
...
function areLayersEqual(
  left: ImageCollageLayerItem[],
  right: ImageCollageLayerItem[],
): boolean {
...
function resolveCollageExportDimensions(
  aspectRatio: string,
  size: ImageCollageNodeData["size"],
): {
  width: number;
  height: number;
} {
  const longEdge =
    EXPORT_LONG_EDGE_BY_SIZE[size] ?? EXPORT_LONG_EDGE_BY_SIZE["1K"];
  const ratio = Math.max(0.01, parseAspectRatio(aspectRatio));
  if (ratio >= 1) {
    return {
      width: longEdge,
...
function reorderLayersByListPosition(
  ...
  const draggedIndex = listOrderedLayers.findIndex(
    (layer) => layer.sourceEdgeId === draggedLayerId,
  );
  const targetIndex = listOrderedLayers.findIndex(
    (layer) => layer.sourceEdgeId === targetLayerId,
  );
...
export const ImageCollageNode = memo(
  ({ id, data, selected, width, height }: ImageCollageNodeProps) => {
    ...
    const imageCacheRef = useRef<
      Map<string, { source: string; image: HTMLImageElement }>
    >(new Map());
    ...
    const normalizedData = useMemo(
      () => normalizeImageCollageNodeData(data),
      [data],
    );
    ...
    const placeLayerAtPosition = useCallback(
      (layerId: string, clientX: number, clientY: number) => {
        ...
        const canvasX = clamp(
          ...
          canvasLayout.x,
          canvasLayout.x + canvasLayout.width,
        );
        const canvasY = clamp(
          clientY - hostRect.top,
          canvasLayout.y,
          canvasLayout.y + canvasLayout.height,
        );
    ...
    useEffect(() => {
      const existingByEdgeId = new Map(
        orderedLayers.map((layer) => [layer.sourceEdgeId, layer] as const),
      );
    ...
    useEffect(() => {
      ...
      const activeLayerIds = new Set(
        orderedLayers.map((layer) => layer.sourceEdgeId),
      );
    ...
    const handleLayerTransform = useCallback(
      (
        ...
        const nextScale = clamp(
          Math.abs(imageNode.scaleX()) / Math.max(baseScale, 0.0001),
          0.05,
          12,
        );
        const centerX =
    ...
    const handleToggleWhiteBackground = useCallback(() => {
      ...
      updateNodeData(id, {
        backgroundMode:
          normalizedData.backgroundMode === "white" ? "transparent" : "white",
      });
    ...
    const handleExport = useCallback(async () => {
      ...
      try {
        const { width: exportWidth, height: exportHeight } =
          resolveCollageExportDimensions(
        ...
        for (const layer of placedLayers) {
          ...
          const image =
            ...
            (await loadImageElement(
              layer.imageUrl || layer.previewImageUrl || "",
            ));
          ...
          context.save();
          context.translate(
            layer.centerX * exportWidth,
            layer.centerY * exportHeight,
          );
    ...
    const handleLayerReorder = useCallback(
      (dragSourceLayerId: string, dropTargetLayerId: string) => {
        const nextLayers = reorderLayersByListPosition(
          orderedLayers,
          dragSourceLayerId,
          dropTargetLayerId,
        );
                  ...
                  <Layer>
                    ...
                    >
                      {placedLayers.map((layer) => {
                        ...
                        const baseScale = resolveImageBaseScale(
                          image,
                          canvasLayout,
                        );
                        return (
                          <KonvaImage
                            ...
                            x={
                              canvasLayout.x +
                              layer.centerX * canvasLayout.width
                            }
                            y={
                              canvasLayout.y +
                              layer.centerY * canvasLayout.height
                            }
                            offsetX={image.naturalWidth / 2}
                            offsetY={image.naturalHeight / 2}
                            scaleX={
                              baseScale * layer.scale * (layer.flipX ? -1 : 1)
                            }
                            scaleY={
                              baseScale * layer.scale * (layer.flipY ? -1 : 1)
                            }
                            ...
                            onDragMove={(event) =>
                              handleLayerDrag(event, layer, "skip")
                            }
                            onDragEnd={(event) =>
                              handleLayerDrag(event, layer, "push")
                            }
                            onTransform={(event) =>
                              handleLayerTransform(event, layer, "skip")
                            }
                            onTransformEnd={(event) =>
                              handleLayerTransform(event, layer, "push")
                            }
                          />
                        );
                      })}
          ...
          <div className="flex w-[280px] max-w-[36%] min-w-[240px] flex-col rounded-xl border border-white/10 bg-black/10 p-2">
            ...
            <div className="ui-scrollbar min-h-0 flex-1 space-y-2 overflow-y-auto pr-1">
              ...
              {listLayers.map((layer, index) => {
                const isActive =
                  layer.sourceEdgeId === normalizedData.selectedLayerId;
                const isDragTarget =
                  dragOverLayerId === layer.sourceEdgeId &&
                  draggedLayerId !== layer.sourceEdgeId;
                ...
                return (
                  <div
                    ...
                    onDrop={(event) => {
                      event.preventDefault();
                      const sourceLayerId =
                        event.dataTransfer.getData(COLLAGE_LAYER_DRAG_MIME) ||
                        draggedLayerId;
                      if (
                        sourceLayerId &&
                        sourceLayerId !== layer.sourceEdgeId
                      ) {
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="truncate text-sm font-medium text-text-dark">
                        {t("node.imageCollage.layerLabel", {
                          index: listLayers.length - index,
                        })}
                      </div>
          ...
          <div className="flex shrink-0 items-center gap-2">
            ...
            >
              <UiSelect
                ...
                onChange={(event) => {
                  updateNodeData(id, {
                    size: event.target.value as ImageCollageNodeData["size"],
                  });
                }}
              >
            ...
            >
              ...
              {isExporting
                ? t("node.imageCollage.exporting")
                : t("node.imageCollage.export")}
            </UiButton>
          </div>
        </div>

        <div
          className={`mt-1 min-h-[18px] text-[10px] leading-4 ${exportError ? "text-red-200" : "text-text-muted"}`}
        >
