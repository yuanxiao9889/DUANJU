...
import { optimizeScriptLocationFields } from '@/features/canvas/application/scriptAssetOptimizer';
import {
  CANVAS_NODE_TYPES,
...
export const ScriptLocationNode = memo(({
  ...
  const getOptimizationSource = useCallback((): ScriptLocationNodeData => ({
    ...data,
  ...
  const persistLocationAssetLibrary = useCallback((nextData: {
    name: string;
    description: string;
    appearances: string[];
  }) => {
    ...
    const nextAsset = toLocationAsset(nextData);
  ...
  const handleSaveEdit = useCallback(() => {
    ...
    persistLocationAssetLibrary(nextData);

    setIsEditing(false);
  }, [data.displayName, editData, id, persistLocationAssetLibrary, updateNodeData]);
  ...
  const { isOptimizing, handleOptimize } = useScriptAssetOptimization({
    validateSource: () => {
      ...
      return source.name.trim() || source.description.trim() || source.appearances.length > 0
        ? null
        : t('scriptNodes.location.optimizeMissingSource');
    },
    onStart: () => setIsEditing(true),
    optimize: (nodes) => optimizeScriptLocationFields(getOptimizationSource(), nodes),
    applyOptimizedResult: (result) => {
      const source = getOptimizationSource();
      persistLocationAssetLibrary({
        name: source.name,
