...
import { AudioLines, LibraryBig, Pause, Play, Sparkles } from "lucide-react";
...
export const VoxCpmUltimateCloneNode = memo(
  ({ id, data, selected }: VoxCpmUltimateCloneNodeProps) => {
    ...
    const handleFieldChange = (
      field: keyof VoxCpmUltimateCloneNodeData,
      value: unknown,
    ) => {
        ...
        >
          <NodeHeader
            ...
            titleText={resolveNodeDisplayName(
              CANVAS_NODE_TYPES.voxCpmUltimateClone,
              data,
            )}
            editable
            ...
            {isOverviewRender ? (
              <div className="rounded-xl border border-white/10 bg-black/10 px-3 py-2.5 text-xs leading-5 text-text-muted">
                ...
                <div className="mt-1 line-clamp-3 whitespace-pre-wrap">
                  {promptTextDraft.trim() ||
                    t("node.voxCpm.promptTextPlaceholder")}
                </div>
              </div>
            ) : (
                ...
                {selectedPresetAsset ? (
                  <div className="mt-3 space-y-2">
                    ...
                    <PresetDetailCard
                      ...
                      actionLabel={
                        presetTranscript
                          ? t("node.voxCpm.applyPresetTranscript")
                          : undefined
                      }
                      onAction={
                        presetTranscript
                          ? handleApplyPresetTranscript
                          : undefined
                      }
                    />
                  </div>
