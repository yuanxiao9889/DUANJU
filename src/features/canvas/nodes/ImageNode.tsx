...
export const ImageNode = memo(
  ({ id, data, selected, type, width }: ImageNodeProps) => {
    ...
    const rawGenerationError =
      typeof (data as { generationError?: unknown }).generationError ===
      "string"
        ? ((data as { generationError?: string }).generationError ?? "").trim()
        : "";
    const generationErrorDetails =
      typeof (data as { generationErrorDetails?: unknown })
        .generationErrorDetails === "string"
        ? (
            (data as { generationErrorDetails?: string })
    ...
    const generationJobId =
      typeof (data as { generationJobId?: unknown }).generationJobId ===
      "string"
        ? ((data as { generationJobId?: string }).generationJobId ?? "").trim()
        : "";
    const generationPhase =
      typeof (data as { generationPhase?: unknown }).generationPhase ===
      "string"
        ? ((data as { generationPhase?: string }).generationPhase ?? "").trim()
        : "";
    const generationFailureStage =
      typeof (data as { generationFailureStage?: unknown })
        .generationFailureStage === "string"
        ? (
            (data as { generationFailureStage?: string })
              .generationFailureStage ?? ""
          ).trim()
        : "";
    const generationStatusText =
      typeof (data as { generationStatusText?: unknown })
        .generationStatusText === "string"
        ? (
            (data as { generationStatusText?: string }).generationStatusText ??
            ""
          ).trim()
        : "";
    ...
    const generationProviderId =
      typeof (data as { generationProviderId?: unknown })
        .generationProviderId === "string"
        ? (
            (data as { generationProviderId?: string }).generationProviderId ??
            ""
          ).trim()
        : "";
    ...
    const collapsedHeight = Math.max(
      explicitHeight ?? compactSize.height,
      resizeConstraints.minHeight,
    );
    ...
    useEffect(() => {
      ...
      const syncImageDimensions = async () => {
        ...
        if (imageWidth === null || imageHeight === null) {
          const dimensions = await detectImageDimensions(
            primaryDimensionSource,
          );
          if (
            disposed ||
        ...
        const previewDimensions = await detectImageDimensions(
          previewDimensionSource,
        ).catch(() => null);
        if (
          disposed ||
        ...
        const originalDimensions = await detectImageDimensions(
          originalDimensionSource,
        ).catch(() => null);
        if (
          disposed ||
          !originalDimensions ||
          (originalDimensions.width === imageWidth &&
            originalDimensions.height === imageHeight)
        ) {
          return;
        }
    ...
    const fallbackImageSource = useMemo(() => {
      ...
      if (imageSource === previewImageUrl) {
        return originalImageUrl && originalImageUrl !== imageSource
          ? originalImageUrl
          : null;
      }

      return previewImageUrl && previewImageUrl !== imageSource
        ? previewImageUrl
        : null;
          ...
          <NodeStatusBadge
            label={
              isSubmittingGeneration
                ? t("nodeStatus.submitting")
                : t("nodeStatus.generating")
            }
            tone="processing"
          />
        );
