...
export const VideoNode = memo(
  ({ id, data, selected, width }: VideoNodeProps) => {
    ...
    const collapsedMinHeight =
      resizeConstraints.minHeight + VIDEO_NODE_CONTROL_BAR_HEIGHT;
    ...
    const resolvedWidth = Math.max(
      resolveNodeDimension(width, mediaMinWidth),
      resizeMinWidth,
    );
    ...
    const resolvedTitle = useMemo(() => {
      const videoFileName =
        typeof data.videoFileName === "string" ? data.videoFileName.trim() : "";
      if (
    ...
    const posterThumbnailSource = useMemo(() => {
    ...
    const seekToNextFrame = useCallback(() => {
      ...
      const newTime = Math.min(
        duration,
        videoRef.current.currentTime + FRAME_TIME,
      );
      videoRef.current.currentTime = newTime;
      setCurrentTime(newTime);
    }, [isPlaying, duration]);
      ...
      (error: unknown): string => {
        ...
        if (
          ...
          normalizedMessage.includes(
            "timed out waiting for video frame data",
          ) ||
    ...
    const handleScreenshot = useCallback(async () => {
      ...
      try {
        ...
        const preferredSource =
          videoRef.current.currentSrc || videoSource || "";
        let dataUrl: string;
        ...
        try {
          ...
          console.warn(
            "[videoNode] direct screenshot capture failed, trying fallback source capture",
            {
          ...
          dataUrl = await captureVideoFrameFromSource(
            preferredSource,
            captureTime,
          );
        }
        ...
        if (!nodePosition) {
          showScreenshotStatus(
            "danger",
            t("node.videoNode.screenshotFailed"),
            4200,
          );
          return;
        }
        ...
        console.error("Failed to capture screenshot:", {
          ...
          id,
          videoSource:
            videoRef.current?.currentSrc || videoSource || data.videoUrl,
        ...
        showScreenshotStatus(
          "danger",
          resolveScreenshotFailureMessage(error),
          4200,
        );
      } finally {
          ...
          {data.videoUrl ? (
            <>
              <div className="flex h-full flex-col overflow-hidden rounded-[var(--node-radius)] bg-[linear-gradient(165deg,rgba(255,255,255,0.05),rgba(255,255,255,0.015))]">
                <div
                  className={`relative min-h-0 flex-1 overflow-hidden bg-black ${flashFrame ? "animate-pulse bg-white/20" : ""}`}
                >
                  {posterSource &&
                  (shouldSuspendMedia ||
                    !isVideoReady ||
                    ...
                    <CanvasNodeImage
                      src={posterSource}
                      overviewSrc={posterThumbnailSource}
            ...
            <label className="block h-full w-full overflow-hidden rounded-[var(--node-radius)] bg-bg-dark">
              <div className="flex h-full w-full cursor-pointer flex-col items-center justify-center gap-3 text-text-muted/85">
                ...
                <span className="text-xs text-text-muted/50">
                  {t("node.videoNode.supportedFormats")}
                </span>
              </div>
            </label>
          )}
