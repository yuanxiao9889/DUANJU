...
import {
  CANVAS_NODE_TYPES,
  SEEDVR2_VIDEO_TARGET_RESOLUTIONS,
  SEEDVR2_VIDEO_UPSCALE_NODE_DEFAULT_HEIGHT,
  SEEDVR2_VIDEO_UPSCALE_NODE_DEFAULT_WIDTH,
  type Seedvr2VideoTargetResolution,
...
type SeedVR2VideoUpscaleNodeProps = NodeProps & {
  id: string;
  data: Seedvr2VideoUpscaleNodeData;
  selected?: boolean;
};
...
function localizeSeedvr2VideoError(
  errorMessage: string,
  t: ReturnType<typeof useTranslation>["t"],
): string {
  const normalized = errorMessage.toLowerCase();
  if (normalized.includes("model files are missing")) {
    return t("node.seedvr2VideoUpscale.modelMissing");
  }
  if (
    normalized.includes("gpu memory is insufficient") ||
    normalized.includes("out of memory")
  ) {
    return t("node.seedvr2VideoUpscale.insufficientMemory");
  }
  if (
    normalized.includes("cuda-capable nvidia gpu") ||
    normalized.includes("requires a cuda-capable")
  ) {
    return t("node.seedvr2VideoUpscale.gpuRequired");
  }
  if (normalized.includes("audio remux failed")) {
    return t("node.seedvr2VideoUpscale.videoMuxFailed");
  }

  return errorMessage;
}

export const SeedVR2VideoUpscaleNode = memo(
  ({ id, data, selected }: SeedVR2VideoUpscaleNodeProps) => {
    ...
    const enabledExtensionIds = useExtensionsStore(
      (state) => state.enabledExtensionIds,
    );
    ...
    const activeExtensionPackage =
      readyExtensionPackage ?? extensionState.pendingPackage;
    ...
    const runtimeTone = resolveRuntimeTone(
      isExtensionReady,
      isExtensionStarting,
    );
    const runtimeProgress = extensionRuntime?.progress ?? 0;
    const currentRuntimeStep =
      activeExtensionPackage?.startupSteps.find(
        (step) => step.id === extensionRuntime?.currentStepId,
      ) ?? null;
    const targetResolution = SEEDVR2_VIDEO_TARGET_RESOLUTIONS.includes(
      data.targetResolution as Seedvr2VideoTargetResolution,
    )
      ? data.targetResolution
      : SEEDVR2_VIDEO_TARGET_RESOLUTIONS[1];

    const inputState = useMemo<VideoInputState>(() => {
      if (incomingSourceNodes.length === 0) {
        return {
          kind: "empty",
          message: t("node.seedvr2VideoUpscale.noInputVideo"),
          previewName: t("node.seedvr2VideoUpscale.waitingForInput"),
      ...
      if (incomingSourceNodes.length > 1) {
        return {
          kind: "multiple",
          message: t("node.seedvr2VideoUpscale.multipleInputVideos"),
          previewName: t("node.seedvr2VideoUpscale.multipleInputTitle"),
          previewImageUrl: null,
        };
      }

      const sourceNode = incomingSourceNodes[0]?.node;
      const source = resolveSingleVideoConnectionSource(sourceNode);
      const previewName = sourceNode
        ? resolveNodeDisplayName(sourceNode.type, sourceNode.data)
        : t("node.seedvr2VideoUpscale.waitingForInput");

      if (!source) {
        return {
          kind: "unsupported",
          message: t("node.seedvr2VideoUpscale.unsupportedInput"),
          previewName,
          previewImageUrl: null,
        };
      }
      ...
      return {
        kind: "ready",
        message: t("node.seedvr2VideoUpscale.inputReady"),
        previewName,
    ...
    const runtimeMessage = useMemo(() => {
      if (isExtensionReady) {
        return t("node.seedvr2VideoUpscale.runtimeReady");
      }

      if (isExtensionStarting) {
        return (
          currentRuntimeStep?.description ??
          t("node.seedvr2VideoUpscale.extensionStarting")
        );
      }

      if (extensionRuntime?.status === "error") {
        return (
          extensionRuntime.error?.trim() ||
          t("node.seedvr2VideoUpscale.runtimeErrored")
        );
      }

      return t("node.seedvr2VideoUpscale.runtimeDisabled");
    }, [
      currentRuntimeStep?.description,
      extensionRuntime?.error,
      extensionRuntime?.status,
      isExtensionReady,
      isExtensionStarting,
      t,
    ]);
    ...
    const helperMessage = useMemo(() => {
      if (isUpscaling) {
        return t("node.seedvr2VideoUpscale.processing");
      }
      ...
      if (!isExtensionReady) {
        ...
        return isExtensionStarting
          ? t("node.seedvr2VideoUpscale.extensionStarting")
          : t("node.seedvr2VideoUpscale.extensionDisabled");
      }
    ...
    const canExecute =
      isExtensionReady && inputState.kind === "ready" && !isUpscaling;
    ...
    const handleResolutionChange = (value: string) => {
      ...
      if (
        !SEEDVR2_VIDEO_TARGET_RESOLUTIONS.includes(
          nextResolution as Seedvr2VideoTargetResolution,
        )
      ) {
        return;
      }
      ...
      updateNodeData(id, {
        targetResolution: nextResolution as Seedvr2VideoTargetResolution,
      });
    };
    ...
    const handleExecute = async () => {
      ...
      updateNodeData(
        ...
        {
          lastError: null,
          statusText: t("node.seedvr2VideoUpscale.persistingInput"),
        },
        { historyMode: "skip" },
      );

      try {
        const localVideoPath = await persistVideoLocally(inputState.videoUrl);

        updateNodeData(
          id,
          {
            statusText: t("node.seedvr2VideoUpscale.processing"),
          },
          { historyMode: "skip" },
        );

        const preparedVideo = await upscaleVideoWithSeedvr2(
          readyExtensionPackage,
          {
            videoPath: localVideoPath,
            targetResolution,
            outputPrefix: `seedvr2-video-${Date.now()}`,
          },
        );

        const resultNodeId = addDerivedVideoNode(
          id,
          ...
          preparedVideo.duration,
          {
            ...
            videoFileName: preparedVideo.videoFileName,
          },
        );

        if (!resultNodeId) {
          throw new Error(t("node.seedvr2VideoUpscale.resultNodeCreateFailed"));
        }
        ...
        updateNodeData(
          ...
          {
            lastError: null,
            statusText: t("node.seedvr2VideoUpscale.completed"),
            lastGeneratedAt: Date.now(),
          },
          { historyMode: "skip" },
        );
      } catch (error) {
        const errorMessage = localizeSeedvr2VideoError(
          resolveErrorMessage(
            error,
            t("node.seedvr2VideoUpscale.processFailed"),
          ),
          t,
        );
        updateNodeData(
    ...
    return (
      ...
      `}
        style={{
          width: SEEDVR2_VIDEO_UPSCALE_NODE_DEFAULT_WIDTH,
          minHeight: SEEDVR2_VIDEO_UPSCALE_NODE_DEFAULT_HEIGHT,
        }}
      ...
      >
        <NodeHeader
          ...
          titleText={resolveNodeDisplayName(
            CANVAS_NODE_TYPES.seedvr2VideoUpscale,
            data,
          )}
          editable
          ...
          rightSlot={
            isUpscaling ? (
              <NodeStatusBadge
                label={t("node.seedvr2VideoUpscale.processingShort")}
                tone="warning"
                animate
              />
            ) : data.lastError ? (
              <NodeStatusBadge
                ...
                label={t("node.seedvr2VideoUpscale.errorShort")}
                tone="danger"
              ...
              <NodeStatusBadge
                icon={<Sparkles className="h-3.5 w-3.5" />}
                label={t("node.seedvr2VideoUpscale.readyShort")}
                tone="processing"
                title={t("node.seedvr2VideoUpscale.generatedAt", {
                  time: formatGeneratedTime(
                    data.lastGeneratedAt,
                    i18n.language,
                  ),
                })}
        ...
        <div className="space-y-3">
            ...
            <div className="flex items-center justify-between gap-3">
              <div>
                <div className="text-xs font-medium text-current">
                  {t("node.seedvr2VideoUpscale.runtimeTitle")}
                </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                {inputState.kind === "ready" ? (
                  <div className="mt-2 space-y-1 text-[11px] leading-4 text-text-muted">
                    <div>
                      {t("node.seedvr2VideoUpscale.sourceAspectRatio", {
                        aspectRatio: inputState.aspectRatio,
                      })}
                    </div>
                    {typeof inputState.duration === "number" ? (
                      <div>
                        {t("node.seedvr2VideoUpscale.sourceDuration", {
                          duration: formatVideoTime(inputState.duration),
                        })}
                      </div>
                    ) : null}
                  </div>
          ...
          <label className="block rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2.5">
            <div className="text-[11px] font-medium uppercase tracking-[0.14em] text-text-muted">
              {t("node.seedvr2VideoUpscale.targetResolution")}
            </div>
            <select
            ...
            >
              {SEEDVR2_VIDEO_TARGET_RESOLUTIONS.map((resolution) => (
                <option key={resolution} value={resolution}>
                  {t("node.seedvr2VideoUpscale.targetResolutionOption", {
                    value: resolution,
                  })}
                </option>
              ))}
          ...
          >
            <span className="inline-flex items-center gap-2">
              ...
              <span>
                {isUpscaling
                  ? t("node.seedvr2VideoUpscale.executing")
                  : t("node.seedvr2VideoUpscale.execute")}
              </span>
            </span>
          </UiButton>
...
SeedVR2VideoUpscaleNode.displayName = "SeedVR2VideoUpscaleNode";
