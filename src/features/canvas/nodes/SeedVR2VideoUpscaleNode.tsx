...
function localizeSeedvr2VideoError(errorMessage: string, t: ReturnType<typeof useTranslation>['t']): string {
...
export const SeedVR2VideoUpscaleNode = memo(({
  ...
  const runtimeMessage = useMemo(() => {
    ...
    if (isExtensionStarting) {
      return currentRuntimeStep?.description ?? t('node.seedvr2VideoUpscale.extensionStarting');
    }

    if (extensionRuntime?.status === 'error') {
      return extensionRuntime.error?.trim() || t('node.seedvr2VideoUpscale.runtimeErrored');
    }
  ...
  const handleResolutionChange = (value: string) => {
    ...
    if (!SEEDVR2_VIDEO_TARGET_RESOLUTIONS.includes(nextResolution as Seedvr2VideoTargetResolution)) {
      return;
    }

    updateNodeData(id, { targetResolution: nextResolution as Seedvr2VideoTargetResolution });
  };
  ...
  const handleExecute = async () => {
        ...
        id,
        {
          lastError:
            extensionRuntime?.status === 'error'
              ? extensionRuntime.error?.trim() || t('node.seedvr2VideoUpscale.runtimeErrored')
              : t('node.seedvr2VideoUpscale.extensionDisabled'),
    ...
    try {
      ...
      const preparedVideo = await upscaleVideoWithSeedvr2(readyExtensionPackage, {
      ...
      const errorMessage = localizeSeedvr2VideoError(
        resolveErrorMessage(error, t('node.seedvr2VideoUpscale.processFailed')),
        t
      );
      updateNodeData(
        id,
  ...
  return (
    ...
    >
      <NodeHeader
        ...
        titleText={resolveNodeDisplayName(CANVAS_NODE_TYPES.seedvr2VideoUpscale, data)}
        editable
          ...
          >
            {SEEDVR2_VIDEO_TARGET_RESOLUTIONS.map((resolution) => (
              <option key={resolution} value={resolution}>
                {t('node.seedvr2VideoUpscale.targetResolutionOption', { value: resolution })}
              </option>
            ))}
          </select>
        </label>
