...
export const DirectorStageNode = memo(({
  ...
  width,
  height,
}: DirectorStageNodeProps) => {
  const { t } = useTranslation();
  const openDirectorStage = useCanvasStore((state) => state.openDirectorStage);
  const resolvedWidth = resolveDimension(width, DIRECTOR_STAGE_NODE_DEFAULT_WIDTH);
  const resolvedHeight = resolveDimension(height, DIRECTOR_STAGE_NODE_DEFAULT_HEIGHT);
  ...
  const objectCount = data.objectCount
    ?? data.project.entities.filter((entity) => !entity.crowdGroupId).length + crowdGroups.length;
  const cameraShotCount = data.cameraShotCount ?? data.project.cameraShots.length;
  const rawActiveShotName =
    data.activeCameraShotName
    ?? data.project.cameraShots.find((shot) => shot.id === data.project.activeCameraShotId)?.name
    ?? t('node.directorStage.freeView');
  const activeShotName =
    rawActiveShotName === 'Shot 1' ? t('node.directorStage.defaultShotName') : rawActiveShotName;
