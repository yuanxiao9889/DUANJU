...
export const ViduVideoResultNode = memo(
  ({ id, data, selected, width }: ViduVideoResultNodeProps) => {
    ...
    const resolvedMinHeight =
      VIDU_VIDEO_RESULT_NODE_MIN_HEIGHT + descriptionPanelHeight;
    ...
    const taskStatusLabel = t(
      `node.viduVideoResult.taskStatuses.${normalizedTaskStatus}`,
    );
        ...
        try {
          ...
          if (!response.video) {
            ...
            await flushCurrentProjectToDiskSafely(
              "saving Vidu video failed requery result",
            );
            if (!options?.suppressErrorDialog) {
          ...
          await flushCurrentProjectToDiskSafely(
            "saving Vidu video requery result",
          );
        } catch (error) {
          ...
          if (options?.scheduleNextPoll) {
            ...
            await flushCurrentProjectToDiskSafely(
              "saving Vidu video requery error",
            );
          }
    ...
    return (
      ...
      >
        ...
        />
        <NodeResizeHandle
          minWidth={VIDU_VIDEO_RESULT_NODE_MIN_WIDTH}
