...
import {
  ...
  VIDU_VIDEO_RESULT_NODE_MIN_HEIGHT,
  VIDU_VIDEO_RESULT_NODE_MIN_WIDTH,
...
import {
  VIDU_VIDEO_RESULT_POLL_INTERVAL_MS,
  queryViduVideoResult,
} from '@/features/vidu/application/viduVideoSubmission';
import { resolveViduErrorContent } from '@/features/vidu/application/viduError';
import { useCanvasStore } from '@/stores/canvasStore';
import { useSettingsStore } from '@/stores/settingsStore';

type ViduVideoResultNodeProps = NodeProps & {
  id: string;
  data: ViduVideoResultNodeData;
  selected?: boolean;
};
...
export const ViduVideoResultNode = memo(({ id, data, selected, width }: ViduVideoResultNodeProps) => {
  ...
  const resolvedTitle = useMemo(
    () => resolveNodeDisplayName(CANVAS_NODE_TYPES.viduVideoResult, data),
    [data]
  );
  const resolvedWidth = Math.max(
    VIDU_VIDEO_RESULT_NODE_MIN_WIDTH,
    Math.round(width ?? VIDU_VIDEO_RESULT_NODE_DEFAULT_WIDTH)
  );
  ...
  const collapsedHeight = Math.max(
    explicitHeight ?? VIDU_VIDEO_RESULT_NODE_MIN_HEIGHT,
    VIDU_VIDEO_RESULT_NODE_MIN_HEIGHT
  );
  const resolvedMinHeight = VIDU_VIDEO_RESULT_NODE_MIN_HEIGHT + descriptionPanelHeight;
  ...
  const taskStatusLabel = t(`node.viduVideoResult.taskStatuses.${normalizedTaskStatus}`);
  const taskStatusNotice =
    normalizedTaskStatus === 'queued'
      ? t('node.viduVideoResult.statusQueued')
      : normalizedTaskStatus === 'running'
        ? t('node.viduVideoResult.statusRunning')
        : null;
  ...
  const handleRequeryResult = useCallback(
    async (options?: { suppressErrorDialog?: boolean; scheduleNextPoll?: boolean }) => {
      ...
      if (!taskId) {
        const message = t('node.viduVideoResult.requeryUnavailable');
      ...
      if (!apiKey) {
        const message = t('node.vidu.apiKeyRequired');
        setStatusNotice(message);
      ...
      if (!options?.suppressErrorDialog) {
        await flushCurrentProjectToDiskSafely('starting Vidu video requery');
      }

      try {
        const response = await queryViduVideoResult({ apiKey, taskId });
        ...
        if (!response.video) {
          const errorMessage =
            response.errorMessage === 'InternalServiceFailure'
              ? t('node.viduVideoResult.internalServiceFailure')
              : response.errorMessage ?? t('node.viduVideoResult.requeryFailed');
          updateNodeData(id, {
            taskId: response.taskId,
            isGenerating: false,
          ...
          await flushCurrentProjectToDiskSafely('saving Vidu video failed requery result');
          if (!options?.suppressErrorDialog) {
        ...
        await flushCurrentProjectToDiskSafely('saving Vidu video requery result');
      } catch (error) {
        const content = resolveViduErrorContent(
          error,
          t('node.viduVideoResult.requeryFailed'),
        ...
        if (options?.scheduleNextPoll) {
          ...
          }, VIDU_VIDEO_RESULT_POLL_INTERVAL_MS * 2);
        } else {
          await flushCurrentProjectToDiskSafely('saving Vidu video requery error');
        }
  ...
  const normalizedLastError =
    data.lastError === 'InternalServiceFailure'
      ? t('node.viduVideoResult.internalServiceFailure')
      : data.lastError ?? null;
  const combinedError = videoError ?? normalizedLastError ?? null;
  const headerStatus = data.isGenerating ? (
    ...
    <NodeStatusBadge
      ...
      label={t('node.viduVideoResult.ready')}
      tone="warning"
    />
  ) : undefined;
  ...
  return (
    ...
    >
            ...
            >
              {videoSource && !shouldSuspendMedia ? (
                ...
                <div className="flex h-full w-full items-center justify-center bg-[radial-gradient(circle_at_top,#1f2937_0%,#0f172a_72%)] text-sm text-text-muted">
                  {showBlockingOverlay || (shouldSuspendMedia && posterSource) ? null : t('node.viduVideoResult.empty')}
                </div>
              )}
            </div>
            {showBlockingOverlay ? (
      ...
      <div className="mt-2 flex min-h-[28px] items-center justify-between gap-2">
        ...
        >
          {t('node.viduVideoResult.requery')}
        </UiButton>
      </div>
      ...
      <Handle
        ...
        className="!h-2.5 !w-2.5 !border-2 !border-surface-dark !bg-accent"
      />
      <NodeResizeHandle minWidth={VIDU_VIDEO_RESULT_NODE_MIN_WIDTH} minHeight={resolvedMinHeight} />
    </div>
  );
});

ViduVideoResultNode.displayName = 'ViduVideoResultNode';
