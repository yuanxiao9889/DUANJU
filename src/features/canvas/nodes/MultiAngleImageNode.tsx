...
import { AlertTriangle, Camera, Loader2, Move3D, Rotate3D, Sparkles } from 'lucide-react';
...
function resolveDirection(angle: number): AngleLabelSet {
  ...
  if (normalized < 67.5) {
    return { prompt: 'front-right quarter view', labelKey: 'frontRight', shortEn: 'front-right' };
  }
  if (normalized < 112.5) {
    return { prompt: 'right side view', labelKey: 'right', shortEn: 'right side' };
  }
  if (normalized < 157.5) {
    return { prompt: 'back-right quarter view', labelKey: 'backRight', shortEn: 'back-right' };
  }
  if (normalized < 202.5) {
    return { prompt: 'back view', labelKey: 'back', shortEn: 'back' };
  }
  if (normalized < 247.5) {
    return { prompt: 'back-left quarter view', labelKey: 'backLeft', shortEn: 'back-left' };
  }
  if (normalized < 292.5) {
    return { prompt: 'left side view', labelKey: 'left', shortEn: 'left side' };
  }
  return { prompt: 'front-left quarter view', labelKey: 'frontLeft', shortEn: 'front-left' };
}
...
function resolveElevation(value: number): AngleLabelSet {
  ...
  if (normalized < 45) {
    return { prompt: 'elevated shot', labelKey: 'elevated', shortEn: 'elevated' };
  }
...
function resolveDistance(value: number): AngleLabelSet {
  ...
  if (normalized < 6) {
    return { prompt: 'medium shot', labelKey: 'medium', shortEn: 'medium shot' };
  }
...
function resolveCameraPosition(horizontalAngle: number, verticalAngle: number, zoom: number): THREE.Vector3 {
...
function resolveUnscaledElementSize(element: HTMLElement): {
  ...
  return {
    width: Math.max(1, Math.round(element.clientWidth || element.offsetWidth || 1)),
    height: Math.max(1, Math.round(element.clientHeight || element.offsetHeight || 1)),
  };
}
...
function MultiAngleScene({
  ...
  const anglesRef = useRef({ horizontalAngle, verticalAngle, zoom, cameraView });
  ...
  useEffect(() => {
    ...
    const backMaterial = new THREE.MeshBasicMaterial({ map: createGridTexture() });
    ...
    const cardFrame = new THREE.LineSegments(
      ...
      new THREE.LineBasicMaterial({ color: 0xe93d82, transparent: true, opacity: 0.9 })
    );
    ...
    const groundRing = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({ color: 0xe93d82, transparent: true, opacity: 0.72 })
    );
    ...
    const azimuthGlow = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({ color: 0xe93d82, transparent: true, opacity: 0.22 })
    );
    ...
    for (let index = 0; index <= 40; index += 1) {
      ...
      elevationArcPoints.push(new THREE.Vector3(
        ELEVATION_ARC_X,
    ...
    const elevationArc = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({ color: 0x00ffd0, transparent: true, opacity: 0.82 })
    );
    ...
    const elevationGlow = new THREE.Mesh(
      new THREE.SphereGeometry(0.22, 16, 16),
      new THREE.MeshBasicMaterial({ color: 0x00ffd0, transparent: true, opacity: 0.22 })
    );
    ...
    const distanceGlow = new THREE.Mesh(
      new THREE.SphereGeometry(0.22, 16, 16),
      new THREE.MeshBasicMaterial({ color: 0xffb800, transparent: true, opacity: 0.26 })
    );
    ...
    const updateImage = async (
      ...
      for (const candidate of imageCandidates) {
        try {
          ...
          const maxSize = 1.55;
          const scaleX = imageAspectRatio > 1 ? maxSize : maxSize * imageAspectRatio;
          const scaleY = imageAspectRatio > 1 ? maxSize / imageAspectRatio : maxSize;
    ...
    let dragging = false;
    let dragTarget: 'azimuth' | 'elevation' | 'distance' | 'orbit' | null = null;
    let startX = 0;
    let startY = 0;
    ...
    const handles = [
      ...
      { mesh: elevationHandle, glow: elevationGlow, name: 'elevation' as const },
    ...
    const handlePointerMove = (event: PointerEvent) => {
      ...
      if (dragTarget === 'azimuth') {
        const azimuthPlane = new THREE.Plane(
          new THREE.Vector3(0, 1, 0),
          0
        );
        if (raycaster.ray.intersectPlane(azimuthPlane, intersect)) {
          let nextAngle = THREE.MathUtils.radToDeg(Math.atan2(intersect.x, intersect.z));
          if (nextAngle < 0) {
            nextAngle += 360;
          }
          onAnglesChangeRef.current({ horizontalAngle: normalizeAzimuth(nextAngle) });
        }
        return;
      }
      ...
      if (dragTarget === 'elevation') {
        ...
        if (raycaster.ray.intersectPlane(elevationPlane, intersect)) {
          ...
          const nextAngle = THREE.MathUtils.radToDeg(Math.atan2(relativeY, intersect.z));
          onAnglesChangeRef.current({ verticalAngle: normalizeElevation(nextAngle) });
        }
        return;
      }
      ...
      onAnglesChangeRef.current({
        horizontalAngle: normalizeAzimuth(startHorizontal - (event.clientX - startX) * 0.5),
        verticalAngle: normalizeElevation(startVertical + (event.clientY - startY) * 0.5),
      });
    };
    ...
    const handlePointerUp = (event: PointerEvent) => {
      ...
      dragging = false;
      dragTarget = null;
      renderer.domElement.style.cursor = anglesRef.current.cameraView ? 'grab' : 'grab';
    ...
    const render = () => {
      ...
      const azimuth = THREE.MathUtils.degToRad(normalizeAzimuth(current.horizontalAngle));
      const elevation = THREE.MathUtils.degToRad(normalizeElevation(current.verticalAngle));
      ...
      const elevationY = SCENE_CENTER.y + ELEVATION_RADIUS * Math.sin(elevation);
      ...
      distanceHandle.position.lerpVectors(SCENE_CENTER, markerPosition, distanceT);
...
export const MultiAngleImageNode = memo(({
  ...
  const selectedModel = useMemo(() => getImageModel(
  ...
  const resolvedCompatibleModelConfig = useMemo(
    () =>
      isStoryboardCompatibleModelId(selectedModel.id)
        ...
        : storyboardCompatibleModelConfig,
    [selectedModel.id, storyboardCompatibleModelConfig, storyboardProviderCustomModels]
  );
  const resolvedApi2OkModelConfig = useMemo(
    () =>
      isStoryboardApi2OkModelId(selectedModel.id)
        ...
        : storyboardApi2OkModelConfig,
    [selectedModel.id, storyboardApi2OkModelConfig, storyboardProviderCustomModels]
  );
  ...
  const requestResolution = selectedModel.resolveRequest({ referenceImageCount: 1 });
  ...
  const statusText =
    error
    ?? (hasMultipleSourceImages ? t('node.multiAngleImage.multiInputHint') : null)
    ?? (sourceImage ? t('node.multiAngleImage.statusHint') : t('node.multiAngleImage.noInput'));
  ...
  const handleAnglesChange = useCallback((nextValue: {
    horizontalAngle?: number;
    verticalAngle?: number;
    zoom?: number;
  }) => {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    const initialRequestAspectRatio =
      selectedAspectRatio.value === AUTO_REQUEST_ASPECT_RATIO
        ? pickClosestAspectRatio(sourceAspectRatioValue, supportedAspectRatioValues)
  ...
  return (
    ...
    >
      ...
      <div className="relative min-h-0 flex-1 overflow-hidden rounded-lg border border-[rgba(255,255,255,0.1)] bg-bg-dark/70">
        <MultiAngleScene
          sourceImageUrl={sourceImage?.imageUrl ?? sourceImage?.previewImageUrl ?? null}
        ...
        <div className="absolute bottom-2 left-2 right-2 grid grid-cols-3 gap-2 rounded-lg border border-[rgba(255,255,255,0.12)] bg-black/35 p-2 backdrop-blur">
          <label className="nodrag nowheel min-w-0 text-[10px] leading-4 text-text-muted">
            ...
            <input
              ...
              onPointerDown={(event) => event.stopPropagation()}
              onChange={(event) =>
                handleAnglesChange({ horizontalAngle: Number(event.currentTarget.value) })
              }
          ...
          <label className="nodrag nowheel min-w-0 text-[10px] leading-4 text-text-muted">
            ...
            <input
              ...
              onPointerDown={(event) => event.stopPropagation()}
              onChange={(event) =>
                handleAnglesChange({ verticalAngle: Number(event.currentTarget.value) })
              }
              className="w-full accent-accent"
            />
          </label>
          <label className="nodrag nowheel min-w-0 text-[10px] leading-4 text-text-muted">
            ...
            <input
              ...
              onPointerDown={(event) => event.stopPropagation()}
              onChange={(event) =>
                handleAnglesChange({ zoom: Number(event.currentTarget.value) })
              }
              className="w-full accent-accent"
            />
          </label>
        ...
        <div className="ui-scrollbar min-w-0 flex-1 overflow-x-auto overflow-y-hidden">
          <div className="flex w-max min-w-full items-center gap-1">
            ...
            <UiChipButton
              ...
              onClick={(event) => {
                event.stopPropagation();
                updateNodeData(id, { cameraView: data.cameraView !== true }, { historyMode: 'skip' });
              }}
            >
              <Camera className="h-4 w-4" />
              <span className="text-[11px]">{t('node.multiAngleImage.cameraView')}</span>
            </UiChipButton>
          </div>
        </div>
