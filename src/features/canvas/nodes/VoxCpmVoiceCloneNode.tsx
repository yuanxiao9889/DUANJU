...
export const VoxCpmVoiceCloneNode = memo(
  ({ id, data, selected }: VoxCpmVoiceCloneNodeProps) => {
    ...
    const connectedReferenceAudio = useMemo(() => {
      const sourceAudioNode = incomingSourceNodes.find(
        ({ node }) =>
    ...
    const cfgValue =
      typeof data.cfgValue === "number"
        ? clamp(data.cfgValue, 0.1, 5)
        : DEFAULT_VOXCPM_CFG_VALUE;
    const inferenceTimesteps =
      typeof data.inferenceTimesteps === "number"
    ...
    const handleFieldChange = (
      field: keyof VoxCpmVoiceCloneNodeData,
      value: unknown,
    ) => {
      updateNodeData(id, { [field]: value }, { historyMode: "skip" });
    };

    const handleApplyPresetHint = () => {
      if (!presetHint) {
        return;
      }

      handleFieldChange("controlText", presetHint);
    };
      ...
      const audioNodeId = addNode(
        ...
        audioNodePosition,
        {
          ...
          voicePresetSource: {
            ...
            sourceGeneration: "voxCpmVoiceClone",
          },
        ...
        >
          <NodeHeader
            ...
            titleText={resolveNodeDisplayName(
              CANVAS_NODE_TYPES.voxCpmVoiceClone,
              data,
            )}
            editable
            ...
            {isOverviewRender ? (
              <div className="rounded-xl border border-white/10 bg-black/10 px-3 py-2.5 text-xs leading-5 text-text-muted">
                ...
                <div className="mt-1 line-clamp-3 whitespace-pre-wrap">
                  {typeof data.controlText === "string" &&
                  data.controlText.trim()
                    ? data.controlText
              ...
              <div className="rounded-xl border border-white/10 bg-black/10 px-3 py-3">
                <div className="mb-3">
                  ...
                  >
                    <option value="">
                      {t("node.voxCpm.presetAssetPlaceholder")}
                    </option>
                ...
                >
                  <option value="">
                    {t("node.voxCpm.referenceAssetPlaceholder")}
                  </option>
                ...
                {selectedPresetAsset ? (
                  <div className="mt-3 space-y-2">
                    ...
                    <PresetDetailCard
                      ...
                      value={
                        presetHint || t("node.voxCpm.presetDescriptionEmpty")
                      }
                      actionLabel={
                        presetHint
                          ? t("node.voxCpm.applyPresetHint")
                          : undefined
                      }
                      onAction={presetHint ? handleApplyPresetHint : undefined}
                    />
                    <PresetDetailCard
                      title={t("node.voxCpm.presetTranscriptTitle")}
                      value={
                        presetTranscript ||
                        t("node.voxCpm.presetTranscriptEmpty")
                      }
                    />
                  </div>
                ) : null}
            ...
            {!isOverviewRender ? (
              <>
                <OptimizableTextAreaField
                  ...
                  value={
                    typeof data.controlText === "string" ? data.controlText : ""
                  }
                  ...
                  optimizeFailedMessage={t(
                    "node.voxCpm.optimizeControlTextFailed",
                  )}
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  ...
                  <SliderField
                    ...
                    onChange={(value) =>
                      handleFieldChange("inferenceTimesteps", value)
                    }
                  />
                </div>
