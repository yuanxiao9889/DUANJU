...
export const BackgroundRemoveNode = memo(
  ({ id, data, selected }: BackgroundRemoveNodeProps) => {
    ...
    const runtimeMessage = useMemo(() => {
      ...
      if (extensionRuntime?.status === "error") {
        return (
          ...
          t("node.backgroundRemove.runtimeErrored")
        );
      }
    ...
    const canExecute =
      isExtensionReady && inputState.kind === "ready" && !isRemoving;
    ...
    const handleExecute = async () => {
      ...
      try {
        ...
        const preparedImage = await removeBackgroundWithRmbg(
        ...
        const errorMessage = resolveErrorMessage(
          error,
          t("node.backgroundRemove.processFailed"),
        );
        updateNodeData(
          id,
          {
      ...
      >
        <NodeHeader
          ...
          titleText={resolveNodeDisplayName(
            CANVAS_NODE_TYPES.backgroundRemove,
            data,
          )}
          editable
