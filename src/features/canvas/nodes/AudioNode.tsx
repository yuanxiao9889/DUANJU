...
export const AudioNode = memo(
  ({ id, data, selected, width }: AudioNodeProps) => {
    ...
    const [playbackDuration, setPlaybackDuration] = useState(() =>
      resolvePlaybackTime(data.duration),
    );
    ...
    const resolvedTitle = useMemo(() => {
      const audioFileName =
        typeof data.audioFileName === "string" ? data.audioFileName.trim() : "";
      if (
    ...
    const presetReferenceText =
      normalizeText(voicePresetSource?.referenceText) ||
      fallbackPresetReferenceText.trim();
    ...
    const targetPresetLibrary =
      assetLibraries.find((library) => library.id === presetLibraryId) ?? null;
    ...
    const isTaskQueued = Boolean(
      isGeneratedTaskNode && !data.audioUrl && queuePosition !== null,
    );
    const isTaskRunning = Boolean(
      isGeneratedTaskNode && !data.audioUrl && data.isGenerating,
    );
    const isTaskFailed = Boolean(
      isGeneratedTaskNode && !data.audioUrl && data.lastError,
    );
    ...
    useEffect(() => {
      ...
      const preferredLibraryId =
        ...
        assetLibraries.some(
          (library) => library.id === currentProjectAssetLibraryId,
        )
          ? currentProjectAssetLibraryId
          : (assetLibraries[0]?.id ?? "");
      ...
      setPresetLibraryId((current) => {
        if (
          current &&
          assetLibraries.some((library) => library.id === current)
        ) {
          return current;
        }
    ...
    useEffect(() => {
      ...
      setPresetSubcategoryId((current) => {
        if (
          current &&
          targetPresetSubcategories.some(
    ...
    const handleSavePreset = useCallback(async () => {
      ...
      try {
        const sourceGeneration =
          normalizeText(voicePresetSource?.sourceGeneration) ||
        ...
        >
                ...
                {shouldSuspendMedia ? (
                  <div className="flex items-center gap-3 text-text-muted">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="truncate text-xs" title={audioMetaLabel}>
                        {audioMetaLabel}
                      </div>
                  ...
                  <>
                    <audio
                      ...
                      onEnded={() => {
                        setIsPlaying(false);
                        setCurrentTime(
                          resolvePlaybackTime(audioRef.current?.duration),
                        );
                      }}
                      onError={() => {
                        setIsPlaying(false);
                    ...
                    <div className="flex items-center gap-3">
                      <button
                        ...
                        title={
                          isPlaying
                            ? t("node.audioNode.pause")
                            : t("node.audioNode.play")
                        }
                      >
                        {isPlaying ? (
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-3">
                          <div className="flex min-w-0 items-center gap-1.5 text-[11px] text-text-muted">
                            ...
                            <span className="truncate" title={audioMetaLabel}>
                              {audioMetaLabel}
                            </span>
                          </div>
                          <div className="shrink-0 text-[11px] tabular-nums text-text-muted">
                            {formatAudioDuration(effectiveCurrentTime)} /{" "}
                            {formatAudioDuration(playbackDuration)}
                          </div>
                        </div>
                  ...
                  <>
                    ...
                    <div className="mt-2 h-2 overflow-hidden rounded-full bg-white/10">
                      <div
                        ...
                        style={{
                          width: `${Math.max(isTaskQueued ? 8 : 0, taskProgress)}%`,
                        }}
                      />
                    </div>
          ...
          {!shouldSuspendMedia ? (
            <NodeDescriptionPanel
              ...
              onChange={(value) =>
                updateNodeData(id, { nodeDescription: value })
              }
            />
          ) : null}
          ...
          >
            <div className="space-y-3">
              <label className="block space-y-1.5">
                ...
                >
                  <option value="">
                    {t("node.audioNode.savePresetSelectLibrary")}
                  </option>
              ...
              <label className="block space-y-1.5">
                ...
                <UiTextArea
                  ...
                  placeholder={t(
                    "node.audioNode.savePresetDescriptionPlaceholder",
                  )}
                />
              </label>
              ...
              <div className="rounded-xl border border-white/10 bg-black/10 px-3 py-2.5">
                ...
                <div className="max-h-28 overflow-auto whitespace-pre-wrap text-sm leading-5 text-text-dark">
                  {presetReferenceText ||
                    t("node.audioNode.savePresetMissingReferenceText")}
                </div>
              </div>
