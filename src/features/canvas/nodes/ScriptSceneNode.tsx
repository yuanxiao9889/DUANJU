...
export const ScriptSceneNode = memo(
  ({ id, data, selected, width, height }: ScriptSceneNodeProps) => {
    const { t } = useTranslation();
    const isOverviewRender = useIsOverviewCanvasRender();
    const updateNodeData = useCanvasStore((state) => state.updateNodeData);
    const setSelectedNode = useCanvasStore((state) => state.setSelectedNode);
    const focusSceneNode = useScriptEditorStore(
      (state) => state.focusSceneNode,
    );
    ...
    const sourceChapterTitle =
      sourceChapterData?.title || sourceChapterData?.displayName || "";
    ...
    const handleGenerateEpisodes = useCallback(
      async (mode: "initial" | "regenerate") => {
        ...
        try {
          ...
          const characterNotes = collectConnectedScriptCharacterNotes(
            id,
            nodes,
            edges,
          );
          const nextEpisodes = await generateEpisodesFromSceneNode(
            data,
            sourceChapterData,
            {
    ...
    return (
      <div
        ...
        onClick={() =>
          openWorkbench(activeSceneNodeId === id ? activeEpisodeId : undefined)
        }
      >
        <CanvasHandle
          type="target"
          ...
          className={SCRIPT_SCENE_HANDLE_CLASS}
        />
        <CanvasHandle
          type="source"
          id="source"
          ...
          <div className="flex items-start gap-3">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2 text-[11px] text-text-muted">
                ...
                <span className="rounded-full bg-bg-dark px-2 py-0.5">
                  {t("script.sceneCatalog.sceneLabel", {
                    number: data.sourceSceneOrder + 1,
                  })}
                </span>
              </div>
          ...
          {!isOverviewRender ? (
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              <button
                ...
                onClick={(event) => {
                  event.stopPropagation();
                  void handleGenerateEpisodes(
                    data.episodes.length > 0 ? "regenerate" : "initial",
                  );
                }}
                className={SCRIPT_SCENE_ACTION_BUTTON_CLASS}
              >
          ...
          <div className="mt-3 flex items-center justify-between gap-2 text-xs text-text-muted">
            ...
            {isGenerating ? (
              <span className="text-text-muted">
                {t("script.sceneWorkbench.generating")}
              </span>
            ) : null}
          </div>
            ...
            >
              {previewEpisodes.length > 0 ? (
                previewEpisodes.map((episode) => {
                  const isActive =
                    activeSceneNodeId === id && activeEpisodeId === episode.id;
