...
export const QwenTtsVoiceDesignNode = memo(
  ({ id, data, selected }: QwenTtsVoiceDesignNodeProps) => {
    ...
    const [isOptimizingVoicePrompt, setIsOptimizingVoicePrompt] =
      useState(false);
    const [voicePromptOptimizationError, setVoicePromptOptimizationError] =
    ...
    const speakingRate =
      typeof data.speakingRate === "number" ? data.speakingRate : 1;
    ...
    const speakingRateDescriptor = t(
      resolveSpeakingRateDescriptorKey(speakingRate),
    );
    ...
    const pendingAudioTaskCountSelector = useMemo(
      () => (state: ReturnType<typeof useCanvasStore.getState>) => {
        ...
        return state.edges
          ...
          .filter((audioData) => {
            if (
              audioData.generationSource !== "ttsVoiceDesign" ||
              audioData.audioUrl
            ) {
              return false;
            }
    ...
    const showBlockingOverlay = Boolean(
      pendingAudioTaskCount > 0 || isOptimizingVoicePrompt,
    );
    ...
    const handleFieldChange = <TKey extends EditableVoiceDesignField>(
      ...
      if (key === "voicePrompt") {
        voicePromptValueRef.current =
          typeof value === "string" ? value : voicePromptValueRef.current;
      }
      updateNodeData(
        id,
        {
    ...
    const handleUndoOptimizedVoicePrompt = useCallback(() => {
      ...
      if (
        voicePromptValueRef.current !==
        lastVoicePromptOptimizationUndoState.appliedPrompt
      ) {
        return;
      }

      const restoredPrompt =
        lastVoicePromptOptimizationUndoState.previousPrompt;
    ...
    const handleGenerate = () => {
      ...
      enqueueQwenTtsAudioGeneration({
        ...
        sourceNodeId: id,
        run: () =>
          generateQwenTtsVoiceDesignAudio(readyExtensionPackage, {
    ...
    return (
      <>
        ...
        >
          <NodeHeader
            ...
            titleText={resolveNodeDisplayName(
              CANVAS_NODE_TYPES.ttsVoiceDesign,
              data,
            )}
          ...
          <div className="space-y-3">
              ...
              <div className="mt-3 rounded-xl border border-white/10 bg-black/10 px-3 py-2.5">
                <div className="mb-2 flex items-center justify-between gap-3">
                  ...
                  <div className="rounded-full border border-white/10 px-2 py-1 text-[11px] text-text-muted">
                    {t("node.qwenTts.connectedCharacterCount", {
                      count: connectedCharacterCount,
                    })}
                  </div>
                </div>
              ...
              {isOverviewRender ? (
                <div className="rounded-xl border border-white/10 bg-black/10 px-3 py-2.5 text-xs leading-5 text-text-muted">
                  <div className="line-clamp-4 whitespace-pre-wrap">
                    {(data.voicePrompt ?? "").trim() ||
                      t("node.qwenTts.voicePromptPlaceholder")}
                  </div>
                </div>
              ) : (
                <>
                  <textarea
                    ...
                    onChange={(event) =>
                      handleFieldChange("voicePrompt", event.target.value)
                    }
                    ...
                    <UiSelect
                      ...
                      onChange={(event: ChangeEvent<HTMLSelectElement>) => {
                        handleFieldChange(
                          "stylePreset",
                          event.target
                            .value as TtsVoiceDesignNodeData["stylePreset"],
                        );
                      }}
            ...
            {combinedError && !isOverviewRender ? (
        ...
        <UiModal
          isOpen={!isOverviewRender && isRecipeModalOpen}
              ...
              <UiSelect
                ...
                onChange={(event: ChangeEvent<HTMLSelectElement>) => {
                  handleFieldChange(
                    "outputFormat",
                    event.target
                      .value as TtsVoiceDesignNodeData["outputFormat"],
                  );
                }}
                className="!h-10 !rounded-xl !border-white/10 !bg-white/[0.04] !px-3 !text-sm"
              >
        ...
        <UiModal
          isOpen={!isOverviewRender && isRhythmModalOpen}
          title={t("node.qwenTts.rhythmTitle")}
