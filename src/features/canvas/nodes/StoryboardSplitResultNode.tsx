...
function SplitResultFrameCard({
  ...
  return (
    <div className="relative bg-bg-dark/85">
      <div
        className="group/frame relative overflow-hidden bg-surface-dark"
      ...
      <div className="min-h-10 border-t border-[rgba(255,255,255,0.12)] bg-bg-dark/90 px-1.5 py-1 text-[11px] text-text-dark">
        <div className="line-clamp-2 break-words text-text-muted/90">
          {noteText}
        </div>
      </div>
...
export const StoryboardSplitResultNode = memo(
  ({ id, data, selected, width, height }: StoryboardSplitResultNodeProps) => {
    ...
    const resolveExportRootDir = useCallback(async (): Promise<
      string | null
    > => {
      const presetPath =
      ...
      return typeof selectedDir === "string" && selectedDir.trim().length > 0
        ? selectedDir
        : null;
    ...
    const handleExportFrames = useCallback(async () => {
      ...
      try {
        ...
        const projectName = sanitizePathSegment(
          currentProjectName ?? "",
          t("node.storyboardSplitResult.defaultProjectName"),
        );
        const outputDir = await join(rootDir, projectName);
        const baseFileName =
          sanitizeExportLabel(projectName, 40) ||
          t("node.storyboardSplitResult.defaultProjectName");
      ...
      isExportingFrames,
      orderedFrames,
      resolveExportRootDir,
      t,
    ]);

    const handleEditFrame = useCallback(
      async (frame: StoryboardFrameItem, index: number) => {
        try {
    ...
    const handleSeparateAllFrames = useCallback(async () => {
      ...
      const frameEntries = orderedFrames
        ...
        .filter(
          (frame): frame is { sourceImage: string; title: string } =>
            frame.sourceImage.length > 0,
        );
        ...
        >
          ...
          >
            ...
            {Array.from({ length: emptyFrameSlotCount }, (_, index) => (
              <div
                key={`split-result-empty-${index}`}
                className="bg-bg-dark/70"
              >
                <div
