...
function normalizeViewerState(data: Panorama360NodeData): PanoramaViewState {
  const yaw =
    typeof data.viewerYaw === "number" && Number.isFinite(data.viewerYaw)
      ? data.viewerYaw
      : DEFAULT_VIEWER_STATE.yaw;
  const pitch =
    typeof data.viewerPitch === "number" && Number.isFinite(data.viewerPitch)
      ? clampPitch(data.viewerPitch)
      : DEFAULT_VIEWER_STATE.pitch;
  const fov =
    typeof data.viewerFov === "number" && Number.isFinite(data.viewerFov)
...
function resolveViewerStateFromCamera(
...
function areViewStatesEqual(
  left: PanoramaViewState,
  right: PanoramaViewState,
): boolean {
  return (
...
function resolveTextureLoadErrorMessage(
  error: unknown,
...
export const Panorama360Node = memo(
  ({ id, data, selected, width }: Panorama360NodeProps) => {
    ...
    const latestViewStateRef = useRef<PanoramaViewState>(
      normalizeViewerState(data),
    );

    const [isTextureLoading, setIsTextureLoading] = useState(false);
    const [isCapturingScreenshot, setIsCapturingScreenshot] = useState(false);
    const [isCapturingOrbitScreenshots, setIsCapturingOrbitScreenshots] =
      useState(false);
    ...
    const resolvedIncomingImageSource = useMemo(() => {
      for (let index = incomingSourceNodes.length - 1; index >= 0; index -= 1) {
        const resolvedSource = resolveSingleImageConnectionSource(
          incomingSourceNodes[index]?.node,
        );
        if (resolvedSource) {
    ...
    const viewerHeight = Math.max(
      180,
      resolvedHeight - VIEWER_CONTROL_BAR_HEIGHT,
    );
    ...
    useEffect(() => {
      const nextImageData = {
        ...
        aspectRatio:
          resolvedIncomingImageSource?.aspectRatio ?? DEFAULT_ASPECT_RATIO,
      };
    ...
    useEffect(() => {
      ...
      const resizeRenderer = () => {
        ...
        const nextWidth = Math.max(
          1,
          Math.round(viewerHostRef.current.clientWidth),
        );
        const nextHeight = Math.max(
          1,
          Math.round(viewerHostRef.current.clientHeight),
        );
    ...
    useEffect(() => {
      ...
      loader.load(
        ...
        (error) => {
          ...
          setIsTextureLoading(false);
          setViewerError(
            resolveTextureLoadErrorMessage(
              error,
      ...
      (viewState?: PanoramaViewState): string => {
        ...
        const captureWidth =
          currentAspect >= 1
            ...
            : Math.max(
                960,
                Math.round(SCREENSHOT_LONG_SIDE_PX * currentAspect),
              );
        const captureHeight =
          currentAspect >= 1
        ...
        const previousIsApplyingExternalView =
          isApplyingExternalViewRef.current;
        ...
        const newNodeId = addNode(
          CANVAS_NODE_TYPES.upload,
          {
            x:
              ...
              40 +
              columnIndex *
                (resultNodeSize.width + SCREENSHOT_RESULT_NODE_HORIZONTAL_GAP),
            y:
              currentNodePosition.y +
              rowIndex *
                (resultNodeSize.height + SCREENSHOT_RESULT_NODE_VERTICAL_GAP),
          },
          {
      ...
      async (event: ReactMouseEvent<HTMLButtonElement>) => {
        ...
        try {
          ...
          await createScreenshotResultNode(
            screenshotDataUrl,
            (screenshotIndex) =>
              t("node.panorama360.screenshotName", {
                name: resolvedTitle,
          ...
          showActionStatus(
            "danger",
            t("node.panorama360.screenshotFailed"),
            3600,
          );
        } finally {
    ...
    const handleOrbitScreenshots = useCallback(
      async (event: ReactMouseEvent<HTMLButtonElement>) => {
        ...
        try {
          ...
          for (
            let index = 0;
            index < ORBIT_SCREENSHOT_ANGLE_OFFSETS.length;
            index += 1
          ) {
          ...
          showActionStatus(
            "danger",
            t("node.panorama360.orbitScreenshotFailed"),
            4200,
          );
        } finally {
