...
import { UiScrollArea } from '@/components/ui/scroll-area';
import {
  CANVAS_NODE_TYPES,
  type CanvasNode,
        ...
        >
          {sortedGroups.map((group) => {
            ...
            return (
              <div
                key={group.id}
                className={`flex items-center gap-2 rounded-xl px-2 py-1.5 transition-colors ${isActive ? 'bg-accent/12' : 'hover:bg-bg-dark/80'}`}
              >
                <button
                  type="button"
                  onClick={() => onLocateGroup(group.id)}
                  className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-accent/12 text-accent transition-colors hover:bg-accent/20"
                ...
                >
                  <div className="truncate text-[13px] font-medium leading-5 text-text-dark">{groupName}</div>
                  <div className="truncate text-[10px] leading-4 text-text-muted">
