...
export function NodeAddToClipLibraryButton({
  ...
  ) || (currentProjectClipLibraryId?.trim() ?? '');
  ...
  const handleConfirmAdd = useCallback(
    async ({ libraryId, folderId }: { libraryId: string; folderId: string }) => {
      if (!currentProjectId || isSubmitting) {
        return;
      }

      setIsSubmitting(true);
      try {
        if ((currentProjectClipLibraryId ?? null) !== libraryId) {
          await setProjectClipLibrary(currentProjectId, libraryId, null);
        }
        ...
        await flushCurrentProjectToDisk();
        await setProjectClipLastFolder(currentProjectId, result.clipFolderId);
    ...
    <>
      ...
      {currentProjectId ? (
        <ClipLibraryTargetPickerDialog
          ...
          currentLibraryId={currentProjectClipLibraryId}
          currentLastFolderId={currentProjectClipLastFolderId}
