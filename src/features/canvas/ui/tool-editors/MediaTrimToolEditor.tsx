...
function normalizeTrimRange(startTime: number, endTime: number, duration: number): {
  ...
  const maxStart = Math.max(0, safeDuration - MIN_TRIM_DURATION);
  const safeStart = clampTime(startTime, 0, maxStart);
  const safeEnd = clampTime(
    endTime,
