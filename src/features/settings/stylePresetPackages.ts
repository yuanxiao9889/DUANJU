...
export function mergeImportedMjStyleCodePackageData(
  ...
  nextPresets.forEach((preset) => {
    const normalizedCode = normalizeMjPersonalizationCodeIdentity(preset.code);
  ...
  for (const importedPreset of importedPresets) {
    const normalizedCode = normalizeMjPersonalizationCodeIdentity(importedPreset.code);
