...
export function updateMjBatchFromTask(
  ...
  return {
    ...batch,
    taskId: task.id,
    action: batch.autoUpscaleChain
      ? batch.action?.trim() || task.action?.trim() || null
      : task.action?.trim() || batch.action || null,
    status: task.status,
    ...
    autoUpscaleChain: batch.autoUpscaleChain ?? null,
  };
}
