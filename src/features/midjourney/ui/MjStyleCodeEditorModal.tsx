...
export function MjStyleCodeEditorModal({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const normalizedCodeIdentity = normalizeMjPersonalizationCodeIdentity(normalizedCode);
      const duplicatePreset =
        presets.find(
          (preset) =>
            normalizeMjPersonalizationCodeIdentity(preset.code) === normalizedCodeIdentity
            && preset.id !== initialPreset?.id
        ) ?? null;
