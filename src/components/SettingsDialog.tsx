...
const STORYBOARD_PROVIDER_GROUP_CONFIGS: ProviderGroupConfig[] = [
  {
    id: 'recommended',
    labelKey: 'settings.providerGroupRecommended',
    providerIds: ['oopii'],
    defaultCollapsed: false,
  },
  {
...
function normalizeSettingsCategory(category: SettingsCategory | 'pricing' | undefined): SettingsCategory {
  if (category === 'releaseNotes' || category === 'pricing') {
    return 'about';
  }
...
function resolveProviderDisplayName(
  ...
  if (tab === 'storyboard') {
    const override = STORYBOARD_PROVIDER_DISPLAY_NAME_OVERRIDES[provider.id];
    if (override) {
      return language.startsWith('zh') ? override.zh : override.en;
    }
  }
...
export function SettingsDialog({
  ...
  useEffect(() => {
    ...
    if (
      initialProviderTab === 'script'
      && scriptProviders.some((provider) => provider.id === initialProviderId)
    ) {
      setSelectedScriptProvider(initialProviderId);
      return;
    }

    if (
      initialProviderTab === 'storyboard'
      && storyboardProviders.some((provider) => provider.id === initialProviderId)
    ) {
      setSelectedStoryboardProvider(initialProviderId);
      return;
    }

    if (
      initialProviderTab === 'mj'
                ...
                <div className="flex-1 flex min-h-0 min-w-0">
                    ...
                    <nav className="flex-1 overflow-y-auto ui-scrollbar">
                      {localProviderTab === 'officialVideo' ? (
                        ...
                        (
                          ...
                          const isRecommendedGroup =
                            localProviderTab === 'storyboard' && group.id === 'recommended';
                              ...
                              >
                                ...
                                <span
                                  className={`min-w-0 flex-1 truncate ${
                                    isRecommendedGroup
                                      ? 'bg-gradient-to-r from-fuchsia-500 to-amber-400 bg-clip-text text-transparent'
                                      : ''
                                  }`}
                                >
                                  {t(group.labelKey)}
                                  ...
                                  {group.providers.map((provider) => {
                                    ...
                                    const isEnabled =
                                      localProviderTab === 'script'
                                        ...
                                        : localProviderTab === 'mj'
                                          ...
                                          : false;
                                    const providerDisplayName = resolveProviderDisplayName(
                                      provider,
                                      localProviderTab,
                                      i18n.language
                                    );
                                    ...
                                    return (
                                      ...
                                      >
                                        ...
                                        />
                                        {isOopiiRecommended && (
                                          <span className="shrink-0 rounded bg-gradient-to-r from-fuchsia-500 to-amber-400 px-1.5 py-0.5 text-[10px] font-medium text-white shadow-sm">
                                            {t('settings.providerRecommended')}
                                          </span>
                                        )}
                                        <span className="min-w-0 flex-1 truncate text-xs">
                                          {providerDisplayName}
                  ...
                  <div className="min-w-0 flex-1 overflow-y-auto ui-scrollbar p-5">
                    {(() => {
                      ...
                      const isOopiiRecommended =
                        (isScriptTab || localProviderTab === 'storyboard')
                        && provider.id === 'oopii';
                        ...
                        <div className="space-y-4">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              ...
                              {isOopiiRecommended && (
                                <span className="rounded bg-gradient-to-r from-fuchsia-500 to-amber-400 px-2 py-0.5 text-[10px] font-medium text-white shadow-sm">
                                  {t('settings.providerRecommended')}
                                </span>
                              )}
                              {hasKey && (
                          ...
                          {!isMjTab ? (
                            <ProviderModelSettingsSection
                              ...
                              storyboardModelOptions={storyboardModelOptions}
