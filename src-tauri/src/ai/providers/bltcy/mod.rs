...
impl BltcyProvider {
    ...
    async fn send_generation_request(
        ...
        let status = response.status();
        let response_text = response.text().await?;
        info!("[鏌忔媺鍥?AI API] Generation response status: {}", status);
        info!("[鏌忔媺鍥?AI API] Generation response: {}", response_text);
        if !status.is_success() {
            return Err(AIError::Provider(format!(
                "鏌忔媺鍥?AI image generations request failed {}: {}",
                status, response_text
            )));
        }
        ...
        serde_json::from_str(&response_text).map_err(|error| {
            AIError::Provider(format!(
                "Failed to parse 鏌忔媺鍥?AI generations response: {}. Response was: {}",
                error, response_text
            ))
        })
    }
...
impl AIProvider for BltcyProvider {
    ...
    async fn generate(&self, request: GenerateRequest) -> Result<String, AIError> {
        ...
        let payload = if Self::is_gpt_image_2_model(&model) {
