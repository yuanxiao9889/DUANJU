...
fn validate_manifest(
    ...
    if manifest.schema != expected_schema {
        if manifest.schema == STYLE_TEMPLATE_PACKAGE_SCHEMA || manifest.schema == MJ_STYLE_CODE_PACKAGE_SCHEMA {
...
fn read_mj_style_code_package(path: &str) -> Result<MjStyleCodePackageFile, String> {
    let value = read_package_value(path)?;
    let manifest = parse_manifest(&value)?;
    validate_manifest(&manifest, MJ_STYLE_CODE_PACKAGE_SCHEMA, MJ_STYLE_CODE_PACKAGE_KIND)?;
    serde_json::from_value::<MjStyleCodePackageFile>(value)
        .map_err(|err| format!("Failed to decode style code package: {}", err))
}

fn read_local_image_asset(source_path: &str, missing_code: &str, display_name: &str) -> Result<(Vec<u8>, PathBuf), String> {
    ...
    let bytes = fs::read(&path).map_err(|_| package_error_with_detail(missing_code, display_name))?;
    Ok((bytes, path))
}
...
pub async fn import_style_template_package(
    ...
    for template in &mut data.templates {
        template.image_url = resolve_imported_asset_path(template.image_url.clone(), &asset_path_by_id)?;
    }
...
pub async fn import_mj_style_code_package(
    ...
    for preset in &mut data.presets {
        preset.image_url = resolve_imported_asset_path(preset.image_url.clone(), &asset_path_by_id)?;
    }
