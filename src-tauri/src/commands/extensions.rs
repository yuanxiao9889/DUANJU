...
fn resolve_extension_search_roots() -> Vec<PathBuf> {
    ...
    let mut register_root = |root: PathBuf| {
    ...
    if let Ok(current_dir) = std::env::current_dir() {
        register_root(current_dir.join("build").join("extensions"));
        register_root(current_dir.join("extension-packages"));
    }

    let cargo_manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    if let Some(workspace_root) = cargo_manifest_dir.parent() {
        register_root(workspace_root.join("build").join("extensions"));
        register_root(workspace_root.join("extension-packages"));
    }

    roots
}

fn validate_discovered_extension_folder(
    folder: &Path,
    manifest: &ExtensionPackageManifest,
) -> bool {
    if manifest.runtime == "python-bridge" {
        return resolve_runtime_entry_paths(folder, manifest).is_ok();
    }
...
#[tauri::command]
pub fn list_local_extension_packages() -> Result<Vec<String>, String> {
    let mut discovered_paths = Vec::new();
    let mut seen_ids = HashSet::new();
    let mut seen_paths = HashSet::new();

    for root in resolve_extension_search_roots() {
        ...
        for entry in entries.flatten() {
            ...
            let folder_name = folder
                ...
                .unwrap_or_default();
            if folder_name.ends_with("-template") {
                continue;
            }

            let manifest = match read_manifest_from_folder(&folder) {
                Ok(manifest) => manifest,
                Err(_) => continue,
            };

            if !validate_discovered_extension_folder(&folder, &manifest) {
                continue;
            }
            ...
            let normalized_path = normalize_discovered_folder_path(&folder);
            if !seen_paths.insert(normalized_path.clone()) {
                continue;
            }

            discovered_paths.push(normalized_path);
        }
    }
...
mod tests {
    ...
    #[test]
    fn local_extension_discovery_finds_complete_packages_only() {
        static CURRENT_DIR_TEST_LOCK: std::sync::OnceLock<std::sync::Mutex<()>> =
        ...
        let create_package = |folder: &Path, extension_id: &str| {
            fs::create_dir_all(folder.join("runtime").join("python"))
        ...
        create_package(&complete_dir, "test-complete");
        create_package(&template_dir, "test-complete-template");

        std::env::set_current_dir(&temp_root).expect("cwd should switch to temp workspace");

        let discovered = list_local_extension_packages().expect("discovery should succeed");
        let complete_path = normalize_discovered_folder_path(&complete_dir);
        let template_path = normalize_discovered_folder_path(&template_dir);

        assert!(
            discovered.iter().any(|path| path == &complete_path),
            "expected local discovery to include {}",
            complete_path
        );
        assert!(
            discovered.iter().all(|path| path != &template_path),
            "template folder should not be returned: {}",
            template_path
        );

        std::env::set_current_dir(original_current_dir).expect("cwd should be restored");
        fs::remove_dir_all(&temp_root).expect("temp workspace should be removed");
    }
