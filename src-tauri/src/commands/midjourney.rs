...
fn normalize_personalization_code_whitespace(value: &str) -> String {
    value
        .replace("\r\n", " ")
        .replace('\n', " ")
        .split_whitespace()
...
fn starts_with_personalization_flag(value: &str) -> bool {
    value
        .split_whitespace()
        .next()
        .map(|token| token.eq_ignore_ascii_case("--p"))
        .unwrap_or(false)
}

fn strip_leading_personalization_flags(value: &str) -> String {
    let normalized = normalize_personalization_code_whitespace(value);
    if !starts_with_personalization_flag(&normalized) {
        return normalized;
    }

    normalized
        .split_whitespace()
        .skip_while(|token| token.eq_ignore_ascii_case("--p"))
        .collect::<Vec<_>>()
        .join(" ")
}
...
fn normalize_personalization_code(value: &str) -> String {
    ...
    let stripped = strip_leading_personalization_flags(&normalized);
...
fn normalize_personalization_code_identity(value: &str) -> String {
    ...
    if starts_with_personalization_flag(&normalized) {
        strip_leading_personalization_flags(&normalized)
    } else {
        normalized
    }
}

fn resolve_personalization_prompt_fragment(value: &str) -> String {
    let normalized = normalize_personalization_code(value);
    if normalized.is_empty() {
        return String::new();
    }
...
fn normalize_personalization_codes(personalization_codes: &[String]) -> Vec<String> {
    ...
    for value in personalization_codes {
        let normalized = normalize_personalization_code(value);
        let identity = normalize_personalization_code_identity(&normalized);
        if normalized.is_empty()
            || identity.is_empty()
            || seen_identities.iter().any(|item| item == &identity)
        {
            continue;
        }
...
fn build_final_prompt(
    ...
    for code in normalize_personalization_codes(personalization_codes) {
        let fragment = resolve_personalization_prompt_fragment(&code);
...
fn normalize_midjourney_task(
    ...
    Ok(MidjourneyTaskDto {
        ...
        image_width: extract_i64_by_pointers(
            payload,
            &[
                "/imageWidth",
                "/result/imageWidth",
                ...
                "/result/properties/imageWidth",
                "/data/properties/imageWidth",
                "/image_width",
                "/result/image_width",
                "/data/image_width",
                "/properties/image_width",
                "/result/properties/image_width",
                "/data/properties/image_width",
                "/width",
                "/result/width",
                "/data/width",
                "/properties/width",
        ...
        image_height: extract_i64_by_pointers(
            payload,
            &[
                "/imageHeight",
                "/result/imageHeight",
                ...
                "/result/properties/imageHeight",
                "/data/properties/imageHeight",
                "/image_height",
                "/result/image_height",
                "/data/image_height",
                "/properties/image_height",
                "/result/properties/image_height",
                "/data/properties/image_height",
                "/height",
                "/result/height",
                "/data/height",
                "/properties/height",
