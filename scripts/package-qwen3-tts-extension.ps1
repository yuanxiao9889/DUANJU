...
if ([string]::IsNullOrWhiteSpace($SourceDir)) {
  $SourceDir = Join-Path $repoRoot 'build\extensions\qwen3-tts-complete'
}

if ([string]::IsNullOrWhiteSpace($OutputFile)) {
  $OutputFile = Join-Path $repoRoot 'build\packages\qwen3-tts-complete-offline.zip'
}
...
$baseFolderName = Split-Path -Leaf $resolvedSourceDir
$files = Get-ChildItem -LiteralPath $resolvedSourceDir -Recurse -Force -File
$totalBytes = ($files | Measure-Object -Property Length -Sum).Sum

Write-Host "[qwen3-tts-extension] packaging source -> $resolvedSourceDir"
Write-Host "[qwen3-tts-extension] packaging target -> $OutputFile"
Write-Host "[qwen3-tts-extension] file count       -> $($files.Count)"
Write-Host "[qwen3-tts-extension] total bytes      -> $totalBytes"
  ...
  try {
    ...
    foreach ($file in $files) {
      ...
      if (($fileIndex % 250) -eq 0 -or $fileIndex -eq $files.Count) {
        ...
        Write-Host "[qwen3-tts-extension] packaged $fileIndex/$($files.Count) files ($progress%)"
      }
    }
  } finally {
    $archive.Dispose()
...
Write-Host "[qwen3-tts-extension] archive ready   -> $OutputFile"
Write-Host "[qwen3-tts-extension] archive size    -> $outputSizeGb GB"
