...
if ([string]::IsNullOrWhiteSpace($SourceDir)) {
  $SourceDir = Join-Path $repoRoot 'build\extensions\voxcpm2-complete'
}

if ([string]::IsNullOrWhiteSpace($OutputFile)) {
  $OutputFile = Join-Path $repoRoot 'build\packages\voxcpm2-complete-offline.zip'
}
...
Write-Host "[voxcpm2-extension] packaging source -> $resolvedSourceDir"
Write-Host "[voxcpm2-extension] packaging target -> $OutputFile"
Write-Host "[voxcpm2-extension] file count       -> $($files.Count)"
Write-Host "[voxcpm2-extension] total bytes      -> $totalBytes"
  ...
  try {
    ...
    foreach ($file in $files) {
      ...
      if (($fileIndex % 250) -eq 0 -or $fileIndex -eq $files.Count) {
        ...
        Write-Host "[voxcpm2-extension] packaged $fileIndex/$($files.Count) files ($progress%)"
      }
    }
  } finally {
    $archive.Dispose()
...
Write-Host "[voxcpm2-extension] archive ready   -> $OutputFile"
Write-Host "[voxcpm2-extension] archive size    -> $outputSizeGb GB"
