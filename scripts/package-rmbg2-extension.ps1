...
if ([string]::IsNullOrWhiteSpace($SourceDir)) {
  $SourceDir = Join-Path $repoRoot 'build\extensions\rmbg2-complete'
}

if ([string]::IsNullOrWhiteSpace($OutputFile)) {
  $OutputFile = Join-Path $repoRoot 'build\packages\rmbg2-complete-offline.zip'
}
...
Write-Host "[rmbg2-extension] packaging source -> $resolvedSourceDir"
Write-Host "[rmbg2-extension] packaging target -> $OutputFile"
Write-Host "[rmbg2-extension] file count       -> $($files.Count)"
Write-Host "[rmbg2-extension] total bytes      -> $totalBytes"
  ...
  try {
    ...
    foreach ($file in $files) {
      ...
      if (($fileIndex % 250) -eq 0 -or $fileIndex -eq $files.Count) {
        ...
        Write-Host "[rmbg2-extension] packaged $fileIndex/$($files.Count) files ($progress%)"
      }
    }
  } finally {
    $archive.Dispose()
...
Write-Host "[rmbg2-extension] archive ready   -> $OutputFile"
Write-Host "[rmbg2-extension] archive size    -> $outputSizeGb GB"
