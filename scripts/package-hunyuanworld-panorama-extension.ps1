...
if ([string]::IsNullOrWhiteSpace($SourceDir)) {
  $SourceDir = Join-Path $repoRoot 'build\extensions\hunyuanworld-panorama-hq-complete'
}

if ([string]::IsNullOrWhiteSpace($OutputFile)) {
  $OutputFile = Join-Path $repoRoot 'build\packages\hunyuanworld-panorama-hq-complete.zip'
}
...
Write-Host "[hunyuanworld-panorama-extension] packaging source -> $resolvedSourceDir"
Write-Host "[hunyuanworld-panorama-extension] packaging target -> $OutputFile"
Write-Host "[hunyuanworld-panorama-extension] file count       -> $($files.Count)"
Write-Host "[hunyuanworld-panorama-extension] total bytes      -> $totalBytes"
  ...
  try {
    ...
    foreach ($file in $files) {
      ...
      if (($fileIndex % 250) -eq 0 -or $fileIndex -eq $files.Count) {
        ...
        Write-Host "[hunyuanworld-panorama-extension] packaged $fileIndex/$($files.Count) files ($progress%)"
      }
    }
  } finally {
    $archive.Dispose()
...
Write-Host "[hunyuanworld-panorama-extension] archive ready   -> $OutputFile"
Write-Host "[hunyuanworld-panorama-extension] archive size    -> $outputSizeGb GB"
