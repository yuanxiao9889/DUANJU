...
func OpenaiHandlerWithUsage(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	if imageErr := extractImageResponseError(responseBody, &usageResp, resp.StatusCode); imageErr != nil {
		return nil, imageErr
	}

	// Only write the image payload back after we've confirmed it is a real
	// success response rather than an upstream error body disguised as HTTP 200.
	service.IOCopyBytesGracefully(c, resp, responseBody)
...
func extractImageResponseError(responseBody []byte, usageResp *dto.SimpleResponse, statusCode int) *types.NewAPIError {
	if usageResp != nil {
		if oaiError := usageResp.GetOpenAIError(); oaiError != nil && (oaiError.Type != "" || oaiError.Message != "" || oaiError.Code != nil) {
	...
	if !hasUsableImageData(imageResp.Data) {
		return types.NewOpenAIError(fmt.Errorf("upstream returned no image data"), types.ErrorCodeEmptyResponse, http.StatusBadGateway)
	}
	return nil
}

func hasUsableImageData(items []dto.ImageData) bool {
